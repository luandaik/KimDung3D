...
function BloodFightManager:onReceiveWarMatixConf( event )
    ...
    for index,roleGmId in pairs(roleList) do
    	...
    	if (role) then
			...
			if mercenary and mercenary.instanceId == roleGmId then
				self:setRoleIndex( 0, mercenary ,1);
    ...
    if data.stations then
		for k,v in pairs(data.stations) do
			...
			if role then
				self:setRoleIndex( (v.index + 1), role ,0);
	    		role.blood_tag 		= 1
				...
				if mercenary and mercenary.instanceId == v.roleId then
					print(v)
					self:setRoleIndex( (v.index + 1), mercenary ,1);
...
function BloodFightManager:setRoleIndex( index,role,isMercenary)
...
function BloodFightManager:changeStrategy(fromPos, toPos, roleGmId)
	...
	local role = nil
	local isMercenary = nil
	local isAddRole = false
	--选中目标不在阵上
	if fromPos == 0 then
		--目标位置是否已经有角色
		if toPos ~= 0  then
			...
			if role then
				self:setRoleIndex(0,role,isMercenary)
				if isMercenary == 0 then
		...
		self:setRoleIndex(toPos,role,isMercenary)
	else
		--下阵
		if toPos == 0  then
			role = CardRoleManager:getRoleByGmid(roleGmId)
		...
		--交换
			...
			role ,isMercenary = self:getRoleByIndex(toPos)
			if role then
				self:setRoleIndex(fromPos,role,isMercenary)
			end
			role = CardRoleManager:getRoleByGmid(roleGmId)
			isMercenary = 0
			if role == nil then
	...
	TFDirector:dispatchGlobalEventWith(self.UPDATE_GENERRAL_LIST, {isAddRole})
