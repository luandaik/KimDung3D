...
ClimbManager.MaxDifficulty = 7      --摩柯崖最大难度
...
function ClimbManager:challengeMountain(mountainId,isOnlySelfPower,fightType)

    local mountainItem = ClimbConfigure:objectByID(mountainId)
    if mountainItem == nil then
    ...
    TFDirector:send(c2s.CLIMB_CHALLENGE_MOUNTAIN, { mountainId ,fightType,isOnlySelfPower} )
end
...
function ClimbManager:showCarbonChooseLayer(index, stage)
    self:requestCarbonStarInfo()
    self.carbonIdx      = index
    self.carbonStage    = stage
end
...
function ClimbManager:carbonStarInfoReceive( event )
    ...
    layer:loadData(self.carbonStage);
    AlertManager:show()
end
