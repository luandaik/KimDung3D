...
function ErrorCodeManager:restart()
	ErrorCodeManager.errorMap = {}
	local date = GetCorrectDate("*t", GetCorrectTime())
	local timeStr = "Error----"..TFPlugins.getUserID().."_"..date.month.."-"..date.day .."_"..date.hour.."_"..date.min.."_"..date.sec..".log"
	ErrorCodeManager.logFileDir = CCFileUtils:sharedFileUtils():getWritablePath().."\\..\\..\\_"
	ErrorCodeManager.logFileDir = ErrorCodeManager.logFileDir .. timeStr
end
...
function ErrorCodeManager:Lua_ErrorWriteFile(errorMsg)
	if CC_TARGET_PLATFORM ~= CC_PLATFORM_WIN32 then
		return
	end
	local _debug_2 = debug.getinfo(2)
	local fileName = ""
	local currentline = -1
	local errorKey = ""
	if (_debug_2) then
		fileName = _debug_2.source
		currentline = _debug_2.currentline
		errorKey = fileName.."_"..currentline
	end
	--self.logFileDir
	local errorInfo = ErrorCodeManager.errorMap[errorKey]
	local bIsWrite = true
	local dateTime = GetCorrectTime()
--	local timeStr = date.month.."_"..date.day .."_"..date.hour.."_"..date.min.."_"..date.sec
	if (errorInfo) then
		if (dateTime - errorInfo < (30)) then
			return
		end
	end
	
	errorInfo = dateTime
	ErrorCodeManager.errorMap[errorKey] = errorInfo
	
	if (not bIsWrite) then
		return
	end
	local f = assert(io.open(ErrorCodeManager.logFileDir, 'a+'))
	local msg = "LUA ERROR: " .. tostring(msg) .. "\n"
    msg = msg .. debug.traceback()
	local arg = {errorMsg, msg}
