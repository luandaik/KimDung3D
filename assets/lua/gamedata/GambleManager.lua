...
function GambleManager:batchBetAutoSuccessNotify(event)
	hideLoading()
	local data = event.data
	self.batchBetAutoCount = data.count  --自动赌石次数。0表示服务器控制
end
...
function GambleManager:pickupSuccessNotify(event)
	print("GambleManager:PICKUP_SUCCESS_NOTIFY--->")
	hideLoading()
	local data = event.data
...
function GambleManager:gamblingItemCacheList(event)
	print("GambleManager:gamblingItemCacheList--->")
	hideLoading()
	local data = event.data
