...
function HeadPicFrameManager:onReceiveInitData( event )
    ...
    for k,v in ipairs(self.unlockedlist) do
        ...
        v.weight_1 = frameData.weight_1
        v.weight_2 = frameData.weight_2
    end
    self.lockedList = event.data.lockedList or {}
    local removeList = {}
    for k,v in ipairs(self.lockedList) do
        local frameData = HeadPicFrameData:objectByID(v.id)
        if frameData == nil or frameData.lock_visible == 0 then
    ...
    for i,v in ipairs(removeList) do
        local index = nil
        for k,u in pairs(self.lockedList) do
            if v == u.id then
                index = k
                break
            end
...
function HeadPicFrameManager:addUnlockedItem(data)
    ...
    local frameData = HeadPicFrameData:objectByID(self.unlockedlist[index].id)
    self.unlockedlist[index].weight_1 = frameData.weight_1
    self.unlockedlist[index].weight_2 = frameData.weight_2
    index = nil;
    for k,v in ipairs(self.lockedList) do
        if v.id == data.id then
            index = k;
...
function HeadPicFrameManager:addLockedItem(data)
    ...
    if frameData.lock_visible ~= 0 then
        self.lockedList[index] = data;
        self.lockedList[index].weight_1 = frameData.weight_1
        self.lockedList[index].weight_2 = frameData.weight_2
    end
    index = nil;
    for k,v in ipairs(self.unlockedlist) do
        if v.id == data.id then
...
function HeadPicFrameManager:getAttribute()
    local attriTable = {}
    local frameId = MainPlayer:getHeadPicFrameId()
    local frameData = HeadPicFrameData:objectByID(frameId)
    if frameData ~= nil then
        if frameData.attribute and frameData.attribute ~= "" then
            local tbl = stringToTable(frameData.attribute,"|")
            for k,v in pairs(tbl) do
                local attr = stringToNumberTable(v,"_")
                local type = attr[1]
                local num = attr[2]
                attriTable[type] = num
            end
