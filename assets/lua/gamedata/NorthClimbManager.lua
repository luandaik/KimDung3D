...
function NorthClimbManager:ctor()
    ...
    self:restart()
    self.maxLength = self:newLength()
end

function NorthClimbManager:newLength()
    local len = 0
    for v in NorthCaveData:iterator() do
...
function NorthClimbManager:onReceiveNorthCaveDetails( event )
    --print("NorthClimbManager:onReceiveNorthCaveDetails===>> ", event.data)
    hideLoading();
    local data = event.data
    ...
    if self.needGetInfo then
        ...
        self:showNorthMountainLayer(true)
    else
        self:mathFloorState()
...
function NorthClimbManager:showNorthMountainLayer(bClose)
    if self.needGetInfo then
...
function NorthClimbManager:mathFloorState()
    ...
    if self.floorMax >= self:newLength() then
        self.floor = self:newLength()
        return 3
    end
