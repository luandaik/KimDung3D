...
NotifyManager.RETURN_NOTIFY_NUM = "NotifyManager.RETURN_NOTIFY_NUM"
NotifyManager.NOTIFY_22_MSG = "NotifyManager.NOTIFY_22_MSG"
NotifyManager.NOTIFY_31_MSG = "NotifyManager.NOTIFY_31_MSG"
...
function NotifyManager:GetCardNotify(event)
	...
	if cardConfig.quality == 5 then
		nameRGBValue = '#ff9c00'
	elseif cardConfig.quality == 6 then
		nameRGBValue = '#ff0000'
	end
...
function NotifyManager:GetEquipNotify(event)
	...
	if equipConfig.quality == 5 then
		nameRGBValue = '#ff9c00'
	elseif equipConfig.quality == 6 then
		nameRGBValue = '#ff0000'
	end
...
function NotifyManager:obtainGemNotify(event)
	...
	if goodsTemplate.quality == 5 then
		nameRGBValue = '#ff9c00'
	elseif goodsTemplate.quality == 6 then
		nameRGBValue = '#ff0000'
	end
...
function NotifyManager:ReceiveNotifyNum(event)
	...
	TFDirector:dispatchGlobalEventWith(NotifyManager.RETURN_NOTIFY_NUM, {})
end
...
function NotifyManager:receiveGoldEggCallback(event)
	...
	local notifyStr = stringUtils.format(rewarddesc, eggInfo.playerName, eggDesc[eggType], nameRGBValue, cardConfig.name, commonReward.number)

	local notifyStrChat = stringUtils.format(chatMsg, eggInfo.playerName, eggDesc[eggType], nameRGBValue, cardConfig.name, commonReward.number)
...
function NotifyManager:receiveNotifyCallback(event)
	...
	elseif msgType == 19 then
		local itemType = tonumber(msgTab[2])
		local itemId = tonumber(msgTab[3])
		local itemNum = tonumber(msgTab[4])
		local floorId = tonumber(msgTab[5])
		local itemData = {
			itemId = itemId,
			type = itemType,
			number = itemNum
		}
		local rewardItem = BaseDataManager:getReward(itemData)
		
	    local tcolor = GetColorStringByQuality(rewardItem.quality)
	    local str = ""
	    if floorId == nil or floorId == 0 then
	    	if itemNum >= 100000 then
		        str = stringUtils.format(localizable.fun_wan_desc,math.floor(itemNum/10000))
		        str = stringUtils.format(localizable.NotifyManager_FationRank_strTempleteChat5,msgTab[1],tcolor,rewardItem.name,str)
		    else
		        str = stringUtils.format(localizable.NotifyManager_FationRank_strTempleteChat5,msgTab[1],tcolor,rewardItem.name,itemNum)
		    end
		else
		    if itemNum >= 100000 then
		        str = stringUtils.format(localizable.fun_wan_desc,math.floor(itemNum/10000))
		        str = stringUtils.format(localizable.NotifyManager_FationRank_strTempleteChat4,msgTab[1],floorId,tcolor,rewardItem.name,str)
		    else
		        str = stringUtils.format(localizable.NotifyManager_FationRank_strTempleteChat4,msgTab[1],floorId,tcolor,rewardItem.name,itemNum)
		    end
		end
	    self:addMessage(str, 1, 1)
	elseif msgType == 20 then
		local notetype = tonumber(msgTab[1])
    	local str = ""
	    if notetype == 0 then
	    	-- local serverId = tonumber(string.split(msgTab[3],"_")[2])
	        -- local serverInfo = SaveManager:getServerInfo(serverId)
	        -- local serverName = ""
	        -- if serverInfo then
	        --     serverName = serverInfo.name
	        -- end
	        -- str = stringUtils.format(localizable.NotifyManager_Peak_strTempleteChat1,serverName,msgTab[2])
			str = stringUtils.format(localizable.NotifyManager_Peak_strTempleteChat1,msgTab[3],msgTab[2])
	    elseif notetype == 1 then
	    	-- local serverId = tonumber(msgTab[3])
    		-- serverId = math.mod(serverId,1000)
    		-- local serverId0 = tonumber(string.split(msgTab[5],"_")[2])
	        -- local serverInfo = SaveManager:getServerInfo(serverId0)
	        -- local serverName = ""
	        -- if serverInfo then
	        --     serverName = serverInfo.name
	        -- end
	        -- str = stringUtils.format(localizable.NotifyManager_Peak_strTempleteChat2,serverId,msgTab[4],serverName)
			local serverId = tonumber(msgTab[3])
			serverId = math.mod(serverId,1000)
			-- local serverId0 = tonumber(string.split(msgTab[5],"_")[2])
		--    local serverInfo = SaveManager:getServerInfo(serverId0)
		...
		--        serverName = serverInfo.name
		--    end
			str = stringUtils.format(localizable.NotifyManager_Peak_strTempleteChat2,serverId,msgTab[4],msgTab[5])
	    end
	    self:addMessage(str, 1, 1)
	elseif msgType == 21 then
	    ...
	    local tcolor = GetColorStringByQuality(rewardItem.quality)
	    local strFormatChat = localizable.NotifyManager_XunBao_strFormatChat
	    local radomIndex = math.random(1, #strFormatChat)
	    local str = stringUtils.format(strFormatChat[radomIndex],msgTab[1],tcolor,rewardItem.name,itemNum)
		
		self:addMessage(str, 1, 1)
	elseif msgType == 22 then
		local serverName = msgTab[1]
		local playerName = msgTab[2]
		local itemType = tonumber(msgTab[3])
		local itemId = tonumber(msgTab[4])
		local itemNum = tonumber(msgTab[5])
		local itemData = {
			itemId = itemId,
			type = itemType,
			number = itemNum
	    ...
	    local tcolor = GetColorStringByQuality(rewardItem.quality)
	    local strFormatChat = localizable.NotifyManager_TianJiLun_strTempleteChat

	    local str = ""
    	if itemNum >= 100000 then
	        str = stringUtils.format(localizable.fun_wan_desc,math.floor(itemNum/10000))
	        str = stringUtils.format(strFormatChat,serverName,playerName,tcolor,rewardItem.name,str)
	    else
	        str = stringUtils.format(strFormatChat,serverName,playerName,tcolor,rewardItem.name,itemNum)
	    end
		self:addMessage(str, 1, 1)
		TFDirector:dispatchGlobalEventWith(self.NOTIFY_22_MSG, {})
	elseif msgType == 23 then
		local playerName = msgTab[1]
		if (not msgTab[2]) or tonumber(msgTab[2]) == 0 then
			msgTab[2] = 1
		end
		local lev = numberToChinese(tonumber(msgTab[2]))
		local levStr = stringUtils.format( localizable.fumo_boss_jieduan, lev )
		local bossId = tonumber(msgTab[3])
		local bossName = ""
		...
		local strFormat = localizable.NotifyManager_WorldBoss_strFormatAdd1
		local str = stringUtils.format( strFormat, playerName, bossName .. levStr, bossName )
		self:addMessage(str, 1, 1)
	elseif msgType == 24 then
		local strFormat = localizable.NotifyManager_RedPacket_strTempleteChat1
		local str = stringUtils.format( strFormat, msgTab[1],msgTab[2],msgTab[3] )
		self:addMessage(str, 1, 1)
		print('msgType == 24')
	elseif msgType == 25 then
		local strFormat = localizable.NotifyManager_RedPacket_strTempleteChat2
		local str = stringUtils.format( strFormat, msgTab[1],msgTab[2],msgTab[3],msgTab[4])
		self:addMessage(str, 1, 1)
		print('msgType == 25')
	elseif msgType == 27 then
		-- 类型 服务器ID 服务器名字 侠客名字 币值 数量
		--print("msgType == 27")
		local backType = tonumber(msgTab[1])
		if backType == 1 then
			--print("msgType == 27===============>>>>1:",context)
			local strFormat = localizable.NotifyManager_11Reback_strTempleteChat1
			local str = stringUtils.format( strFormat, msgTab[3],msgTab[4],msgTab[5],msgTab[6])

			strFormat = localizable.NotifyManager_11Reback_strTempleteChat1_0
			local strChat = stringUtils.format( strFormat, msgTab[3],msgTab[4],msgTab[5],msgTab[6])

			self:addMessage(str, 1, 1)
			self:sendMsgToChat(strChat,EnumChatType.Server,true)
		elseif backType == 2 then
			--print("msgType == 27===============>>>>2:",context)
			local strFormat = localizable.NotifyManager_11Reback_strTempleteChat2
			local str = stringUtils.format( strFormat,msgTab[4],msgTab[5],msgTab[6])
			self:addMessage(str, 1, 1)
		end
	elseif msgType == 28 then
	    ...
	    local tcolor = GetColorStringByQuality(rewardItem.quality)
	    local strFormatChat = localizable.NotifyManager_Xiakexing_strFormatChat
	    local radomIndex = math.random(1, #strFormatChat)
	    local str = stringUtils.format(strFormatChat[radomIndex],msgTab[1],tcolor,rewardItem.name,itemNum)
	...
	elseif msgType == 31 then
		local msg = {
			serverName = msgTab[1],
			playerName = msgTab[2],
			_type = tonumber(msgTab[3]),
			killNum = tonumber(msgTab[4]),
			icon = tonumber(msgTab[5]),
			head = tonumber(msgTab[6]),
			serverId = tonumber(msgTab[7]),
		}
		TFDirector:dispatchGlobalEventWith(self.NOTIFY_31_MSG, msg)

	elseif msgType == 34 then
		--琅环福地
		local goodsInfo = string.split(msgTab[2], '|')
		local goodsData = {}
		local str = ""
		for k, v in pairs(goodsInfo) do
			local strData = string.split(v, '_')
			local data = {
				type = EnumDropType.GOODS,
				itemId = tonumber(strData[2]),
				number = tonumber(strData[3])
			}

			local itemInfo = BaseDataManager:getReward(data)
			if itemInfo.quality == 2 then
				nameRGBValue = '#008000'
			elseif itemInfo.quality == 3 then
				nameRGBValue = '#0000FF'
			elseif itemInfo.quality == 4 then
				nameRGBValue = '#800080'
			elseif itemInfo.quality == 5 then
				nameRGBValue = '#ff9c00'
			elseif itemInfo.quality == 6 then
				nameRGBValue = '#ff0000'
			end
			if (k >= 2) then
				str = str .. ","
			end
			local addStr = stringUtils.format([[<font color="{p1}" fontSize="26">{p2}</font>]],nameRGBValue,itemInfo.name.."x"..itemInfo.number)
			str = str .. addStr
		end		

		local str = stringUtils.format(localizable.NotifyManager_LangHuan_tip1,msgTab[1], str)
		self:addMessage(str, 1, 1)
	end
end
...
function NotifyManager:clearMsgByType( _type )
	local removeList = {}
	for msg in self.MessageArray:iterator() do
		if msg.showMsgType == _type then
			removeList[#removeList + 1] = msg
		end
	end
	for k,v in pairs(removeList) do
		self.MessageArray:removeObject(v)
	end
end
