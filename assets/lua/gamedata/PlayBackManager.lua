    ...
    if next(list)~= nil then
        for k,v in pairs(list) do
            if self.taskList and self.taskList[v.taskid] ~= nil then
                ...
                if not self:checkTaskIsOpen( v.taskid ) then
                    self.taskList[v.taskid].state = 2
                end
            end
        end
    end
...
function PlayBackManager:checkTaskIsOpen(id)
    for v in self.taskConfig:iterator() do
        if v.id == id and v.show and v.show == 1 then
            return true
        end
    end
    return false
...
function PlayBackManager:playerBackOpenSatus()
    local isopen = false
    local verLock = VersionLockData:objectByID(EnumVersionLockType.PlayerBack_Lock)
