...
SevenDaysManager.UpdateRedPoint = "SevenDaysManager.UpdateRedPoint"
...
function SevenDaysManager:ctor()
    ...
    TFDirector:addProto(s2c.SEVEN_DAYS_PROMPT, self, self.onSevenDaysPromptMsg)
...
function SevenDaysManager:reset_24()
    -- self.halpShopOpenFlag = {}
end
...
function SevenDaysManager:filterTaskData(dayIndex)
    ...
    for v in self.DayTaskData:iterator() do
        if v.tab ~= titleType and v.tab ~= 0 then
            titleType = v.tab
...
function SevenDaysManager:enterSevenDaysLayer1()
...
function SevenDaysManager:checkRedPointWithDayIndex(dayIndex)
    ...
    local checkHalfShop = self:checkRedPointWithTitle(dayIndex, 16)
    if checkHalfShop then
        return true
    end

    for v in sevenDaysConfig:iterator() do
        if v.day == dayIndex and v.tab ~= 16 and v.tab ~=0  then
            local task = nil
            task = self.taskStatusList:objectByID(v.id)
...
function SevenDaysManager:checkRedPointWithTitle(dayIndex, titleType)
    ...
    if titleType == 16 then
        --特殊处理半价商品的红点显示
        if self.halpShopOpenFlag[dayIndex] == 0 then
            return false
        end
        for v in sevenDaysConfig:iterator() do
...
function SevenDaysManager:updateSevenDaysTimer()
    ...
    if status == 1 then
        RegisterTime = RegisterTime + 3600 * 24 * self.constDayNum
    else
        RegisterTime = RegisterTime + 3600 * 24 * (self.constDayNum + 3)
    end
...
function SevenDaysManager:setHalfShopOpen( dayIndex )
    -- self.halpShopOpenFlag[dayIndex] = true
    showLoading()
    TFDirector:send(c2s.SEVEN_DAYS_PROMPT, {dayIndex})
end

function SevenDaysManager:onSevenDaysPromptMsg( event )
    self.halpShopOpenFlag = event.data.sevenPrompt
    hideLoading()
    TFDirector:dispatchGlobalEventWith(SevenDaysManager.UpdateRedPoint, {})
end

--获取开服大礼14天的奖励信息
function SevenDaysManager:get14DaysJiangliInfo()
    local jiangliType = 3000
    --self.taskStatusList:objectByID(v.id)
    local jiangLiInfo = {}
    for v in sevenDaysConfig:iterator() do
        if (v.type == jiangliType) then
            local dayData = {}
            dayData.id = v.id
            dayData.day = v.day
            dayData.title = v.title
            -- 道具奖励
            dayData.itemInfo = RewardConfigureData:GetRewardItemListById(v.reward_id).m_list[1]     
            local tmpData = self.taskStatusList:objectByID(v.id)
            --如果时间没到,服务器是没有数据的
            if (tmpData == nil) then
                tmpData = {}
                tmpData.state = 0
            else
                self.currOpenServerDayS = v.day
            end
            dayData.state = tmpData.state
            jiangLiInfo[dayData.day] = dayData
        end
    end
    return jiangLiInfo
end
...
--获取开服大礼累计登录的奖励信息
function SevenDaysManager:get14DaysLeiJiInfo()
    local jiangliType = 3100
    --self.taskStatusList:objectByID(v.id)
    local leiJiInfo = {}
    local loginDays = 0
    local index = 1
    for v in sevenDaysConfig:iterator() do
        if (v.type == jiangliType) then
            ...
            leijIData.id = v.id
            -- 道具奖励
            leijIData.itemInfo = RewardConfigureData:GetRewardItemListById(v.reward_id).m_list[1]
            --如果时间没到,服务器是没有数据的
            local tmpData = self.taskStatusList:objectByID(v.id)
            if (tmpData == nil) then
                tmpData = {}
                ...
                loginDays = tmpData.currstep
            end
            leijIData.state = tmpData.state

            leiJiInfo[index] = leijIData   
            index = index + 1
        end

    end
    return loginDays, leiJiInfo
end

function SevenDaysManager:openKaiFuDaLiLayer()
    local layer = AlertManager:addLayerByFile('lua.logic.gameactivity.FourteenDayLayer')
    AlertManager:show();
end

function SevenDaysManager:isKaiFuDaLiRedPoint()
    local isShow = false
    local _, leiJiInfo = self:get14DaysLeiJiInfo()
    local dayInfo = self:get14DaysJiangliInfo()
