...
function SkillLevelData:findInfo( skillID , level )
	...
	local spellInfo = FightManager:GetCacheSkillBaseInfo(skillID)
	if spellInfo == nil then
		print("技能信息有误 skillID ,level == ",skillID,level)
		return nil
	end
	
	if spellInfo.bible_key_skill == 0 then
		return self:_findNormalSkillLevelInfo( spellInfo , level )
	else
		return self:_findBibleKeySkillLevelInfo( spellInfo , level )
	end
end

function SkillLevelData:_findNormalSkillLevelInfo( spellInfo , level )
	if level <= 0 then
		return nil
	end
	if level >= MainPlayer:getRoleSkillMax() then
		level = MainPlayer:getRoleSkillMax()
	end
	local level_info = SkillAttributeData:objectByID(level)
	...
	if tbl.type == 2 or tbl.type == EnumFightSkillType.type_15 then
...
function SkillLevelData:_findBibleKeySkillLevelInfo( spellInfo , level )
	if level >= MainPlayer:getRoleSkillMax() then
		level = MainPlayer:getRoleSkillMax()
	end
	local level_info = BibleKeyBuffData:BibleKeyBuffData(spellInfo.id,level)
	local tbl = clone(spellInfo)
	tbl.level = level
	if type(tbl.target_num ) == "string" then
		tbl.target_num = GetVauleByStringRange( tbl.target_num , level)
	...
	if tbl.attr_add_absolute ~= "" then
		...
		if level_info and level_info.attributes and level_info.attributes ~= "" then
			local attributes = GetAttrByString(level_info.attributes)
	...
	if tbl.effect_value ~= "" then
		tbl.effect_value = splitLevelValue(tbl.effect_value ,1 )
	else
		tbl.effect_value = 0
	end

	if tbl.effect_rate ~= "" then
		tbl.effect_rate = splitLevelValue(tbl.effect_rate ,1)
	else
		tbl.effect_rate = 0
	end

    if level_info and level_info.extra and level_info.extra ~= "" then
    	local extraList = GetAttrByString(level_info.extra, "|", ",")
    	for k,v in pairs(extraList) do
    		if k == 1 then
    			tbl.effect_value = tbl.effect_value + v
    		elseif k == 2 then
    			tbl.effect_rate = tbl.effect_rate + v
    		end
    	end
    end
	
	if tbl.triggerSkill_rate ~= "" then
		tbl.triggerSkill_rate = splitLevelValue(tbl.triggerSkill_rate ,1)
	else
		tbl.triggerSkill_rate = 0
	end
	if tbl.trigger_hp ~= "" then
		tbl.trigger_hp = splitLevelValue(tbl.trigger_hp ,1)/100
	else
		tbl.trigger_hp = 0
	end

	if tbl.extra_hurt ~= 0 then
		tbl.extra_hurt = math.floor(tbl.extra_hurt * 1)
	end
	if tbl.buff_targetnum ~= "" then
		tbl.buff_targetnum = GetVauleByStringRange( tbl.buff_targetnum , 1)
	else
		tbl.buff_targetnum = 0
	end
	if tbl.buff_rate ~= "" then
		tbl.buff_rate = splitLevelValue(tbl.buff_rate ,1)
	else
		tbl.buff_rate = 0
	end
	...
	--extra_buffid = 0,extra_targe_type = 1,extra_buff_targetnum = 1,extra_buff_rate="10000_0",extra_buff_formula = 1
	if tbl.extra_buff_targetnum ~= "" then
		tbl.extra_buff_targetnum = GetVauleByStringRange( tbl.extra_buff_targetnum , 1)
	else
		tbl.extra_buff_targetnum = 0
	end
	if tbl.extra_buff_rate ~= "" then
		tbl.extra_buff_rate = splitLevelValue(tbl.extra_buff_rate ,1)
	else
		tbl.extra_buff_rate = 0
	end
	-----------------------
	...
	if tbl.immune ~= "" then
		tbl.immune = splitImmuneAttr(tbl.immune , 1)
	else
		tbl.immune = {}
	end

	if tbl.effect_extra ~= "" then
		tbl.effect_extra = splitImmuneAttr(tbl.effect_extra , 1)
	else
		tbl.effect_extra = {}
	end

	if tbl.be_effect_extra ~= "" then
		tbl.be_effect_extra = splitImmuneAttr(tbl.be_effect_extra , 1)
	else
		tbl.be_effect_extra = {}
	end

	tbl.power = tbl.power_zhanli 
	if level_info and level_info.power then
		tbl.power = tbl.power + level_info.power
	end
	return tbl
end
...
function SkillLevelData:findBuffInfo( buffId , level )
	...
	if buffInfo == nil then
		print("buff信息有误 buffId:",buffId," level: " ,level)
		return nil
	end
	if level >= MainPlayer:getRoleSkillMax() then
		level = MainPlayer:getRoleSkillMax()
	...
	if tbl.params ~= "" then
		local _type = tbl.type
		if _type == EnumFightBufferType.type_513 or _type == EnumFightBufferType.type_1 or _type == EnumFightBufferType.type_502 then
