...
--是否是小包资源且有未下载资源
function tTriggerFunction:getSmallPacketResource( conditions )
	if conditions.smallPacket then
		if TFPlugins.isCompletePack == nil or TFPlugins.isCompletePack == true then
			return false
		end
		local lenth = #DownResourceManager.downState
		for i=1,lenth do
			local status = DownResourceManager.downState[i]
			if status == 0 then
				return true
			end
		end
