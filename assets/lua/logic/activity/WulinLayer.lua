...
function WulinLayer.tableCellAtIndex(table, idx)
    ...
    if activity.id == WulinManager.TAP_Peak then
        local redPointState = PeakManager:checkRedPoint()
        CommonManager:setRedPoint(cell.sprite, redPointState,"peakHaveTimes",ccp(70,80))
    elseif  activity.id == WulinManager.TAP_Hero  then
        local redPointState = HeroMeetingManager:getRedPointState()
        CommonManager:setRedPoint(cell.sprite, redPointState,"peakHaveTimes",ccp(70,80))
    end
