...
function AssistFightLayer:drawFateCell(node)
    ...
    if fateDataNode.type == 1 then --1为角色 2为缘分id
        ...
        local icon1,icon2 = {},{}
        local icon = {icon1,icon2}
        local list = {img_bg1,img_bg2}
        for i = 1,2 do
            for k = 1,4 do
                icon[i][k] = TFDirector:getChildByPath(list[i], "bg_icon"..k)
                icon[i][k].img_icon = TFDirector:getChildByPath(icon[i][k], "img_icon")
                icon[i][k].txt_num = TFDirector:getChildByPath(icon[i][k], "txt_num")
                icon[i][k]:setVisible(false)
            end
        end
        local fatelist  = string.split(fate.target,'|')
        for i =1 ,2 do
            for k,v in pairs(fatelist) do
                local temp = string.split(v,',')  
                icon[i][k]:setVisible(true) 
                local info 
                print(temp)
                if (tonumber(temp[1])) == 1 then
                    info = RoleData:objectByID(tonumber(temp[2]))
                    icon[i][k]:setTexture(GetColorIconByQuality(info.quality))
                    icon[i][k].img_icon:setTexture(info:getIconPath())
                elseif (tonumber(temp[1])) == 2 then
                    info = ItemData:objectByID(tonumber(temp[2]))
                    icon[i][k]:setTexture(GetColorIconByQuality(info.quality))
                    icon[i][k].img_icon:setTexture(info:GetPath())
                end
            end
        end
        ...
        details2:setText(fate.details)
         --
        local richtextHB1 = Public:createRichtextHB(details1)
        local richtextHB2 = Public:createRichtextHB(details2)
        richtextHB1:setText(stringUtils.richText(fate.details))
        richtextHB2:setText(stringUtils.richText(fate.details))
    end
    ...
    if MainPlayer:getLevel() < 50 then
        ...
        for i=1,6 do
            if gridList[i] then
                ...
                --     end
                -- end
                local rankIndex = math.floor((tonumber(level) - 1) / 5) + 1
                local currRank  = math.floor((tonumber(level) - (rankIndex - 1) * 5))
                for j=1,5 do
                    local uiNode = self.qiheIcon[i].bg
                    local starIcon = TFDirector:getChildByPath(uiNode, "icon_lv"..j)
                    if level < j then
...
function AssistFightLayer:showQiheDetailList()
    ...
    local attrList,subAttrList = AssistFightManager:getQihePreviewInfo(self.LineUpType)
    ...
    for i,v in ipairs(subAttrList) do
        ...
        for i,info in ipairs(temp) do
            if (v.attrActiveId and info.attrActiveId == v.attrActiveId) then
                info.activeValue    = info.activeValue + v.activeValue
                info.power          = info.power + v.power
                isAdd = true
            end
            if (v.attrPassiveId and info.attrPassiveId == v.attrPassiveId) then
                info.passiveValue = info.passiveValue + v.passiveValue
                if (not isAdd) then
                    info.power          = info.power + v.power
                end
                isAdd = true
            end
            local des = string.split(v.des, '#')
            if (isAdd and des[3]) then
                info.des = des[1]..math.floor(math.abs(info.activeValue) / 100)..des[3]
            end
        end
        if (not isAdd) then
            temp[i] = {}
            --if (v.attrActiveId) then
                temp[i].attrActiveId = v.attrActiveId or 0
                temp[i].activeValue = v.activeValue or 0
                temp[i].power = v.power
            --end
            --if (v.attrPassiveId) then
                temp[i].attrPassiveId = v.attrPassiveId or 0
                temp[i].passiveValue = v.passiveValue or 0
                local des = string.split(v.des, '#')
                if (des[3]) then
                    local value = 0
                    if (temp[i].passiveValue ~= 0) then
                        value = value + tonumber(temp[i].passiveValue)
                    end
                    if (temp[i].activeValue ~= 0) then
                        value = value + tonumber(temp[i].activeValue)
                    end
                    temp[i].des = des[1]..math.floor(math.abs(value) / 100)..des[3]
                else
                    temp[i].des = v.des
                end
           -- end
    ...
    for idx,val in pairs(attrList) do
        ...
        if (isPercentAttr(tonumber(idx))) then
...
function AssistFightLayer.tableCellAtIndex(table, idx)
    ...
    if item1 or item2 then
        ...
        if (isPercentAttr(item1.idx)) then
        ...
        if item2 then
            ...
            if (isPercentAttr(item2.idx)) then
        ...
        local subItem = self.subAttrTbl[idx - len]
        attr1:setText(subItem.des)
        attr2:setVisible(false)
    end
