...
function SkyBookDetailsLayer:refreshUI()
    ...
    local _quality = SkyBookManager:GetUpQuality(data.quality, data.tupoLevel)
    self.btn_icon:setTextureNormal(GetColorIconByQuality(_quality))
    ...
    if data:getGemPos(1) and data:getGemPos(1) ~= 0 then
        local gemId = data:getGemPos(1)
        self.img_baoshicao:setVisible(true)
        local item1 = ItemData:objectByID(gemId)
        self.img_gem:setTexture(item1:GetPath())
    end

    if data:getGemPos(2) and data:getGemPos(2) ~= 0 then
        local gemId = data:getGemPos(2)
