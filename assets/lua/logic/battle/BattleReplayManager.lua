...
function BattleReplayManager:ExecuteAction()
	TFDirector:currentScene().fightUiLayer:SetCurrActionNum(self.nCurrActionIndex)
	...
	print("第"..self.nCurrRoundIndex.."回合  第"..self.nCurrActionIndex.."动作")
...
function BattleReplayManager:OnActionEnd()
	...
	battleRoleMgr.addHpPercentRole[false] = battleRoleMgr:GetSkillAddMaxHpByRole(false)
	battleRoleMgr.addHpPercentRole[true] = battleRoleMgr:GetSkillAddMaxHpByRole(true)
	self:ExecuteAction()
end
...
function BattleReplayManager:GetAttackOrder()
	...
	for i=self.nCurrActionIndex,#battleActionList.action do
		...
		local num = #orderList
		if actionInfo.type ~= 3 and attackRole ~= nil and attackRole:IsLive() and num < maxNum and attackRole:GetSummoned() == 0 then
			orderList[num+1] = {}
			orderList[num+1].fightRole = attackRole
			if actionInfo.type == 2 then
				orderList[num+1].bManualAction = true
			else
				orderList[num+1].bManualAction = false
			end
			orderList[num+1].nCurrActionIndex = i
		end
	end
...
function BattleReplayManager:addTargetBuff(actionInfo)
	...
	if actionInfo.lostState then
		for i=1,#actionInfo.lostState  do
			local state = actionInfo.lostState[i]
			local target = battleRoleMgr:GetRoleByGirdIndex(state.position)
			if target and state.repeatNum == 0 then
				target:RemoveBuffById(state.stateId)
			end
