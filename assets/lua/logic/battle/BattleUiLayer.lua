...
function BattleUiLayer:initUI(ui)
	...
	self.chatBtn:setVisible(false)
	for i=1,2 do
		local pet = battleRoleMgr:GetPetByIndex(i)
		if pet then
...
function BattleUiLayer:registerEvents()	
	...
	if KingOfWarManager:checkNeedShowSkip() ~= true then
		self.breakBtn:setVisible(true)
	else
		self.breakBtn:setVisible(false)
	end
...
function BattleUiLayer:SetCurrActionNum(nActionNum)
				...
				onComplete = function ()
					for i=1,5 do
						...
						if iconInfo == nil then
							...
							self.orderIcon[i].nCurrActionIndex = orderInfo.nCurrActionIndex
...
function BattleUiLayer:GetAttackIcon(orderInfo)
	for i=1,5 do
		local iconInfo = self.orderIcon[i]
		if iconInfo ~= nil and iconInfo.fightRole == orderInfo.fightRole and iconInfo.nCurrActionIndex == orderInfo.nCurrActionIndex then
			return iconInfo
		end
	end
end
