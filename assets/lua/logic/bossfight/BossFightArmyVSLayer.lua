...
function BossFightArmyVSLayer:registerEvents()
   ...
   self.btn_close:setClickAreaLength(100)
    ...
    self.btn_quanli.logic = self
    self.btn_quanli.index = 2
    self.btn_quanli:addMEListener(TFWIDGET_CLICK, audioClickfun(self.onBtnAttackClickHandle),1)
...
function BossFightArmyVSLayer.onBtnAttackClickHandle(sender)
    ...
    if index == 2 then
        local openVip = ConstantData:getValue("BossFight.Quanli.NeedVIP");
        
        if MainPlayer:getVipLevel() < openVip then
            local msg =  stringUtils.format(localizable.bossFightMainLayer_Quanli,openVip);
        ...
        end,
        {
           -- title=localizable.common_tips,
            msg =  localizable.bossFightMainLayer_QuanLiText,
...
function BossFightArmyVSLayer:Attack(index)
    local bossId   = self.BossData.bossId

    print("开始挑战")
    if BossFightManager:requestAttackBoss(bossId,index-1) == true then
        AlertManager:close(AlertManager.TWEEN_NONE)
    end
end
