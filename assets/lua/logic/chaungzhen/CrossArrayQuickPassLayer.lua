-- client side CrossArrayQuickPassLayer.lua
--[[
 * @Description: 扫荡
 ]]
...
local CrossArrayQuickPassLayer = class("CrossArrayQuickPassLayer", BaseLayer);

CrossArrayQuickPassLayer.LIST_ITEM_HEIGHT = 140; 

CREATE_SCENE_FUN(CrossArrayQuickPassLayer);
CREATE_PANEL_FUN(CrossArrayQuickPassLayer);

function CrossArrayQuickPassLayer:ctor(data)
    self.super.ctor(self,data);
    
    self:init("lua.uiconfig_mango_new.crossarray.CAQuickPassReslutListLayer");
end

function CrossArrayQuickPassLayer:initUI(ui)
    ...
    self.mubiaoNum      = TFDirector:getChildByPath(self.bg_mubiao, 'LabelBMFont_CrossArrayQuickPassLayer_1')
...
function CrossArrayQuickPassLayer:loadData(data, curFightIndex)
    self.itemlist = data
...
function CrossArrayQuickPassLayer:onShow()

    self.super.onShow(self)
    self:refreshBaseUI();

    -- self.txt_shuoming:setText(stringUtils.format(localizable.Cr ossArrayQuickPassLayer_fight_index ,self.curFightIndex))
end

function CrossArrayQuickPassLayer:refreshBaseUI()

end
function CrossArrayQuickPassLayer:refreshUI()
...
function CrossArrayQuickPassLayer:loadTotolList()
...
function CrossArrayQuickPassLayer:loadRewardList()
    ...
    for index,reslutItem in pairs(self.itemlist) do
        ...
        reslut_node:setPosition(ccp(0,(#self.itemlist - index) * CrossArrayQuickPassLayer.LIST_ITEM_HEIGHT))
        self.list_reward:addChild(reslut_node)
        reslut_node:setVisible(false);
        reslut_nodeArr[index] = reslut_node;
    end

    self.list_reward:setInnerContainerSize(CCSizeMake(self.list_reward:getSize().width , #self.itemlist * CrossArrayQuickPassLayer.LIST_ITEM_HEIGHT + 20))
    self.list_reward:scrollToYTop(0);
    self.list_reward:setInnerContainerSizeForHeight(#self.itemlist * CrossArrayQuickPassLayer.LIST_ITEM_HEIGHT + 20)
    ...
    self.change = function()
        ...
        -- local newOffset = ccp(oldOffset.x,oldOffset.y + CrossArrayQuickPassLayer.LIST_ITEM_HEIGHT);
...
--添加玩家节点
function CrossArrayQuickPassLayer:loadRewardNode(reslut_node,reslutItem,index)
    local txt_index = TFDirector:getChildByPath(reslut_node, 'txt_index');
    txt_index:setText(stringUtils.format(localizable.common_index_ceng,index))
...
function CrossArrayQuickPassLayer:removeUI()
    self.super.removeUI(self);
    TFDirector:removeTimer(self.reardTimeId);
end

function CrossArrayQuickPassLayer.onSkipClickHandle(sender)
...
function CrossArrayQuickPassLayer:onSumComplete()
...
function CrossArrayQuickPassLayer:registerEvents()
...
function CrossArrayQuickPassLayer:removeEvents()
    self.sendData = nil
...
return CrossArrayQuickPassLayer;
