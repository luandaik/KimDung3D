...
function ClimbCarbonListLayer:initUI(ui)
    ...
    for i=1,3 do
        ...
        local index                 = self:getByStageIndex(i)
        ...
        self.carbonNode[i].directional_sensitivity = false
        if carbonItem.directional_sensitivity and carbonItem.directional_sensitivity == 1 then
            self.carbonNode[i].directional_sensitivity = true
        end
        
        armature:setPosition(ccp(180,310-30))
...
function ClimbCarbonListLayer:getByStageIndex(stage)
...
function ClimbCarbonListLayer:refreshUI()
    for i=1,3 do
        ...
        local directional_sensitivity = self.carbonNode[i].directional_sensitivity;
        if carbonItem:isOpen() then
            if directional_sensitivity then
...
function ClimbCarbonListLayer:onAttackcarbon(index)
    ...
    if carbonItem:isOpen() then
        ...
        --      -- ClimbManager:showCarbonDetailLayer(index)
            ClimbManager:showCarbonChooseLayer(index, carbonItem.stage)
        -- end
    else
        ClimbManager:showCarbonNotOpenLayer(carbonItem.stage)
    end
end
