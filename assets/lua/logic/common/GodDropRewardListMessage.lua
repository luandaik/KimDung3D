...
local GodDropRewardListMessage = class("GodDropRewardListMessage", BaseLayer);

CREATE_SCENE_FUN(GodDropRewardListMessage);
CREATE_PANEL_FUN(GodDropRewardListMessage);

local ShowMaxNum = 8

function GodDropRewardListMessage:ctor(data)
    self.super.ctor(self,data);
    self:init("lua.uiconfig_mango_new.common.RewardListMessage");
end

function GodDropRewardListMessage:initUI(ui)
    ...
    self.bg:setTexture("ui_new/common/img_reward_bg3.png")
...
function GodDropRewardListMessage:addEffect(effectName)
...
function GodDropRewardListMessage:loadData(rewardList,isShowTxt)
...
function GodDropRewardListMessage:setTextSHow( txt )
    self.txt_cishu:setVisible(true)
    self.txt_cishu:setText(txt)
end

function GodDropRewardListMessage:showGetHero()
...
function GodDropRewardListMessage:onShow()
    self.super.onShow(self)
    self:refreshBaseUI();
end

function GodDropRewardListMessage:refreshBaseUI()

end

function GodDropRewardListMessage:refreshUI()
    -- if not self.isShow then
    --     return;
    -- end
    print("GodDropRewardListMessage:refreshUI-------------111")
    ...
    if self.tableView == nil then
        ...
        -- tableView:addMEListener(TFTABLEVIEW_TOUCHED, GodDropRewardListMessage.tableCellTouched)
        tableView:addMEListener(TFTABLEVIEW_SIZEFORINDEX, GodDropRewardListMessage.cellSizeForTable)
        tableView:addMEListener(TFTABLEVIEW_SIZEATINDEX, GodDropRewardListMessage.tableCellAtIndex)
        tableView:addMEListener(TFTABLEVIEW_NUMOFCELLSINTABLEVIEW, GodDropRewardListMessage.numberOfCellsInTableView)
    ...
    print("GodDropRewardListMessage:refreshUI-------------222")
end


function GodDropRewardListMessage.cellSizeForTable(table,idx)
    return 150,115
end

function GodDropRewardListMessage.tableCellAtIndex(table, idx)
...
function GodDropRewardListMessage:addTag(node,num)
...
function GodDropRewardListMessage.numberOfCellsInTableView(table)
    local num = table.logic.rewardList:length()
    if num > ShowMaxNum then
        num = num+1
    end
...
function GodDropRewardListMessage:removeUI()
   self.super.removeUI(self);
end

--注册事件
function GodDropRewardListMessage:registerEvents()
    self.super.registerEvents(self);

    ADD_ALERT_CLOSE_LISTENER(self,self.btn_ok);
end

function GodDropRewardListMessage:removeEvents()
...
return GodDropRewardListMessage;
