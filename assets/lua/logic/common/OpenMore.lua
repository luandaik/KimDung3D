...
function OpenMore:ctor(more_info)
    self.super.ctor(self,more_info)
    self.id = more_info.itemid
    self.param = more_info.param
  --   if maxNum then
		-- self.maxNum = maxNum
  --   end
...
function OpenMore:initUI(ui)
	...
	local txt_info			= TFDirector:getChildByPath(ui, 'txt_info')
	...
	self.txt_info = Public:createRichtextHB(txt_info, "", EnumRichtextType.ShopOrUse)
	--为按钮绑定处理逻辑属性，指向self
...
function OpenMore:setIconInfo( itemInfo )
	...
	self.txt_info:setText(stringUtils.richText(itemInfo.describe2))
end

function OpenMore:setNChooseMBoxIconInfo( id )
	local giftPackData = GiftPackData:objectByID(id)
	if giftPackData == nil then
		return
	end
    local giftGoodsList = split(giftPackData.goods, "|")
    local index = self.param[1]
    if giftGoodsList == nil then
		return
	end
    local giftData = giftGoodsList[index]
    ...
    self.txt_name:setText(reward.name)
    self.img_icon:setTexture(reward.path)
    self.img_quality:setTexture(GetColorIconByQuality(reward.quality))
    self.txt_info:setText(stringUtils.richText(reward.desc))
end
...
function OpenMore:initInfo( id )
	...
	if BagManager:isNChooseMBox( id ) then
		self:setNChooseMBoxIconInfo( id )
	else
...
function OpenMore.useButtonClickHandle(sender)
	...
	if BagManager:isNChooseMBox( self.id ) then
		BagManager:goChoice(self.id, self.param,self.choiceNum)
		AlertManager:close()
	else
