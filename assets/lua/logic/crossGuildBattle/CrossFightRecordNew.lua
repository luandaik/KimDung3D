-- client side CrossFightRecordNew.lua
--[[
...
local CrossFightRecordNew = class("CrossFightRecordNew",BaseLayer)

function CrossFightRecordNew:ctor(data)

	self.super.ctor(self, data)
	self:init("lua.uiconfig_mango_new.faction.FactionRecord")
end

function CrossFightRecordNew:initUI( ui )
    ...
    self.atkPanel_Buff = TFDirector:getChildByPath(atkGuildNode, "panel_buff")
    self.atkPanel_Buff:setVisible(false)
    self.atkBtn_Buff = TFDirector:getChildByPath(atkGuildNode, "btn_buff")
    self.atkTxt_Buff = TFDirector:getChildByPath(atkGuildNode, "txt_num")
    self.atkBtn_Buff:setVisible(false)
    self.atkTxt_Buff:setVisible(false)
    self.defGuild = {}
    ...
    self.defPanel_Buff = TFDirector:getChildByPath(defGuildNode, "panel_buff")
    self.defPanel_Buff:setVisible(false)
    self.defBtn_Buff = TFDirector:getChildByPath(defGuildNode, "btn_buff")
    self.defTxt_Buff = TFDirector:getChildByPath(defGuildNode, "txt_num")
    self.defTxt_Buff:setVisible(false)
    self.defBtn_Buff:setVisible(false)
...
function CrossFightRecordNew:removeUI()
	self.super.removeUI(self)
end

function CrossFightRecordNew:onShow()
    self.super.onShow(self)
end

function CrossFightRecordNew:registerEvents()
    ...
    self.atkBtn_Buff:addMEListener(TFWIDGET_CLICK, audioClickfun(self.btnBuffClick))
    self.defBtn_Buff:addMEListener(TFWIDGET_CLICK, audioClickfun(self.btnBuffClick))
...
function CrossFightRecordNew:removeEvents()
...
function CrossFightRecordNew:dispose()
	self.super.dispose(self)
end

function CrossFightRecordNew.cellSizeForTable(table,idx)
    if idx == 1 then
        return 23,760
    else
        return 80,760
...
function CrossFightRecordNew.numberOfCellsInTableView(table)
    local self = table.logic
    
    return self.currFightIndex
end

function CrossFightRecordNew.tableCellAtIndex(table, idx)
...
function CrossFightRecordNew:setCellData(panel,idx, showTime)
    ...
    if atkInfo then
        ...
        local RoleIcon = RoleData:objectByID(atkInfo.useIcon)
    ...
    if defInfo then
        ...
        local RoleIcon = RoleData:objectByID(defInfo.useIcon)
...
function CrossFightRecordNew:showDataDetails(panels, idx)
...
function CrossFightRecordNew:setFightRound( index )
    self.selectTeamIndex = 1
    self:refreshTeamInfoByIndex()
end

function CrossFightRecordNew:refreshTeamInfoByIndex()
    
    self.isEnd,self.currFightIndex = CrossGuildBattleManager:getStateByTeamIndex(self.selectTeamIndex)
    ...
    self.fightRecordData = CrossGuildBattleManager:getTeamInfoByIndex(self.selectTeamIndex)
    self.atkMemberInfo = CrossGuildBattleManager:getAtkMemberInfo(self.selectTeamIndex)
    self.defMemberInfo = CrossGuildBattleManager:getDefMemberInfo(self.selectTeamIndex)
...
function CrossFightRecordNew:getPlayerInfo( desTable, playerId)
    print('desTable = ',desTable)
...
function CrossFightRecordNew:refreshMemberHead()
    ...
    for k,v in pairs(self.atkMember) do
        ...
        if data then
            ...
            print('data = ',data)
            local RoleIcon = RoleData:objectByID(data.useIcon)
...
function CrossFightRecordNew:refreshGuildInfo()

    local atkGuildBuf = false
    local defGuildBuf = false
    local isEnd,currFightIndex = CrossGuildBattleManager:getStateByTeamIndex(self.selectTeamIndex)
    if isEnd or currFightIndex > 1 then
        local fightRecord = CrossGuildBattleManager:getTeamInfoByIndex(self.selectTeamIndex)
    ...
    local atkGuildData,defGuildData = CrossGuildBattleManager:getGuildDataInfo()
    for i=1,3 do
    ...
    if atkGuildData and atkGuildData.name then
        ...
        if atkGuildData.peakAddition and atkGuildData.peakAddition > 0 then
            self.atkPanel_Buff:setVisible(false) -- pck tag 2017.2.15
            self.atkBtn_Buff.addNum = atkGuildData.peakAddition
            self.atkTxt_Buff:setText(atkGuildData.peakAddition * 0.01 .. '%')
        else
            self.atkPanel_Buff:setVisible(false)
        end
        -- local bufIndex = 1
    ...
    if defGuildData and defGuildData.name then
        ...
        if defGuildData.peakAddition and defGuildData.peakAddition > 0 then
            self.defPanel_Buff:setVisible(false) -- pck tag 2017.2.15

            self.defBtn_Buff.addNum = defGuildData.peakAddition
            self.defTxt_Buff:setText(defGuildData.peakAddition * 0.01 .. '%')
        else
            self.defPanel_Buff:setVisible(false)
        end
        if defGuildBuf then
...
function CrossFightRecordNew:refreshBtnView()

end

function CrossFightRecordNew.leftBtnClick( btn )
...
function CrossFightRecordNew.rightBtnClick( btn )
    local self = btn.logic
...
function CrossFightRecordNew.btnBuffClick( btn )
    local num = btn.addNum or 0
    PeakManager:openBuffLayer0(num)
end

function CrossFightRecordNew:moveFont()   
    if self.fontMoveTime then
        return
    else
        for i=1,11 do
...
function CrossFightRecordNew.onReplayClick( btn )
...
function CrossFightRecordNew.closeBtnClick( btn )
    local self = btn.logic
...
function CrossFightRecordNew.tableScroll(table)
    local self = table.logic
