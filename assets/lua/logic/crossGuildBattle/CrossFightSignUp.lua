-- client side CrossCrossFightSignUp.lua
--[[
 * @Description: 跨服争锋战报名
 ]]
...
local CrossFightSignUp = class("CrossFightSignUp",BaseLayer)

local MaxQueueNum = 3
local leaderData = {}
local memberData = {}
local btnTexture = {}
function CrossFightSignUp:ctor(data)

	self.super.ctor(self, data)
	self:init("lua.uiconfig_mango_new.faction.FactionSignUp")
end

function CrossFightSignUp:initUI( ui )
...
function CrossFightSignUp:removeUI()
	self.super.removeUI(self)
end

function CrossFightSignUp:onShow()
    self.super.onShow(self)
end

function CrossFightSignUp:registerEvents()
...
function CrossFightSignUp:removeEvents()
...
function CrossFightSignUp:dispose()
	self.super.dispose(self)
end

--added by wuqi
function CrossFightSignUp.btnBianjiClickHandle(btn)
    ...
    local layer = require("lua.logic.crossGuildBattle.CrossFightSignUpEditor"):new()
...
function CrossFightSignUp.btnLeaderAddClick( btn )
    --选择精英
    if FactionManager:getPostInFaction() ~= 1 then
        --toastMessage("权限不够")
	toastMessage(localizable.common_no_power)
    ...
    local layer = require("lua.logic.crossGuildBattle.CrossFightSignUpChoose"):new()
    AlertManager:addLayer(layer,AlertManager.BLOCK_AND_GRAY,AlertManager.TWEEN_1)
    layer:dataReady(btn.idx)
    AlertManager:show()
end

function CrossFightSignUp.btnSignUpClick( btn )
    --报名
    local self = btn.logic
    local teamIndex = btn.idx
    if self:checkMySelfInTeam(teamIndex) then
        --取消报名
        CrossGuildBattleManager:unapplyCrossGuildBattle()
    else
        CrossGuildBattleManager:applyCrossGuildBattle(teamIndex-1)
    end
end

function CrossFightSignUp:checkMySelfInTeam(teamIndex)
...
function CrossFightSignUp.cellSizeForTable(table,idx)
    return 80,230
end

function CrossFightSignUp.numberOfCellsInTableView(table)
    local self = table.logic;
    local idx = table.idx
    return 10
end

function CrossFightSignUp.tableCellAtIndex(table, idx)
...
function CrossFightSignUp:updateMemberInfo()
    for i=1,MaxQueueNum do        
        leaderData[i] = {}
        memberData[i] = {}
...
function CrossFightSignUp:dataReady()
    for i=1,MaxQueueNum do
        leaderData[i] = CrossGuildBattleManager:getLeaderDataByIndex(i)
        memberData[i] = CrossGuildBattleManager:getMemberDataByIndex(i)
...
function CrossFightSignUp:showLeaderData()
