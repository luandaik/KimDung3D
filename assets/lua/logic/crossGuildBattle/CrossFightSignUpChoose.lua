-- client side CrossFightSignUpChoose.lua
--[[
...
local CrossFightSignUpChoose = class("CrossFightSignUpChoose",BaseLayer)

local memberData = {}
function CrossFightSignUpChoose:ctor(data)

	self.super.ctor(self, data)
	self:init("lua.uiconfig_mango_new.faction.FactionSignUpChoose")
end

function CrossFightSignUpChoose:initUI( ui )
...
function CrossFightSignUpChoose:removeUI()
	self.super.removeUI(self)
end

function CrossFightSignUpChoose:onShow()
    self.super.onShow(self)
end

function CrossFightSignUpChoose:registerEvents()
...
function CrossFightSignUpChoose:removeEvents()
...
function CrossFightSignUpChoose:dispose()
	self.super.dispose(self)
end

function CrossFightSignUpChoose.cellSizeForTable(table,idx)
    return 78,290
end

function CrossFightSignUpChoose.numberOfCellsInTableView(table)
    return 5
end

function CrossFightSignUpChoose.tableCellAtIndex(table, idx)
...
function CrossFightSignUpChoose:dataReady(queueIndex)

    memberData = CrossGuildBattleManager:getMemberDataByIndex(queueIndex)
    local leaderData = CrossGuildBattleManager:getLeaderDataByIndex(queueIndex)
...
function CrossFightSignUpChoose.btnOkClick( btn )
    ...
    if playerId ~= 0 then
        CrossGuildBattleManager:updateEliteCrossGuildBattle(self.queueIndex - 1, playerId)    
    end
end

function CrossFightSignUpChoose.btnXuanzeClick( btn )
    local self = btn.logic
    self.currSelectIdx = btn.idx
    self.tabView:reloadData()
end
return CrossFightSignUpChoose