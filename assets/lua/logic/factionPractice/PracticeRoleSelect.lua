...
function PracticeRoleSelect:cellInfoSet( panel, idx )
    ...
    if panel.boundData == nil then
        ...
        panel.img_zhan:setTexture("ui_new/array/js_chuzhan_icon2.png")
        ...
        panel.img_zhu = TFDirector:getChildByPath(panel, 'img_zhu')
    ...
    if  roleItem then
        ...
        if roleItem.pos and roleItem.pos ~= 0 then
            panel.img_zhan:setVisible(true);
        else
            panel.img_zhan:setVisible(false);
        end
        --添加助战标识
        if AssistFightManager:isInAssist( self.currLineUpType, roleItem.gmId ) then
            panel.img_zhu:setVisible(true)
        else
            panel.img_zhu:setVisible(false)
        end
