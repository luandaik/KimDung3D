...
function FactionFightMessage:initUI(ui)
    ...
    for i = 1, 16 do
        ...
        self.bangpai_item[i].panel_buff = TFDirector:getChildByPath(node, "panel_buff")
        self.bangpai_item[i].panel_buff:setVisible(false)
        self.bangpai_item[i].btn_buff = TFDirector:getChildByPath(node, "btn_buff")
        self.bangpai_item[i].txt_buff = TFDirector:getChildByPath(node, "txt_num")
        self.bangpai_item[i].btn_buff:setVisible(false)
        self.bangpai_item[i].txt_buff:setVisible(false)
...
function FactionFightMessage:initData()
    ...
    for i = 1,8   do
         if self.warInfos[1][i] then
            ...
            headTemp.peakAddition = v.atkPeakAddition
            ...
            tailTemp.peakAddition = v.defPeakAddition
...
function FactionFightMessage:bWin(guildId, Round)
    ...
    if Round < 1 or Round > 4 then
        ...
        if self.warInfos[Round] == nil then
            return bWin
        end        
...
function FactionFightMessage:showQiZhi(i)
    ...
    if self.teamInfoList[i].peakAddition and self.teamInfoList[i].peakAddition > 0 then
        self.bangpai_item[i].panel_buff:setVisible(true)
        self.bangpai_item[i].btn_buff.addNum = self.teamInfoList[i].peakAddition
        self.bangpai_item[i].txt_buff:setText(self.teamInfoList[i].peakAddition * 0.01 .. '%')
    else
        self.bangpai_item[i].panel_buff:setVisible(false)
    end
end
...
function FactionFightMessage:registerEvents()
    ...
    for i = 1, 16 do
        self.bangpai_item[i].btn_buff:addMEListener(TFWIDGET_CLICK, audioClickfun(self.btnBuffClick))
    end
...
function FactionFightMessage.onBtnCheck(btn)
    ...
    if self.warInfos[round] and self.warInfos[round][index] then
        ...
        atkInfo.peakAddition = self.warInfos[round][index].atkPeakAddition
    end
    ...
    if self.warInfos[round] and self.warInfos[round][index] then
        ...
        defInfo.peakAddition = self.warInfos[round][index].defPeakAddition
    end
...
function FactionFightMessage.btnBuffClick( btn )
