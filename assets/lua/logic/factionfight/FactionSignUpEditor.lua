...
local FactionSignUpEditor = class("FactionSignUpEditor", BaseLayer)

FactionSignUpEditor.ZHENLIE_COUNT = 3
FactionSignUpEditor.ZHENLIE_ROLE_COUNT = 10
FactionSignUpEditor.FENGE_COUNT = 12

FactionSignUpEditor.leaderData = {}
FactionSignUpEditor.memberData = {}
FactionSignUpEditor.memberData1 = {}
...
function FactionSignUpEditor:ctor(data)
...
function FactionSignUpEditor:initUI(ui)
...
function FactionSignUpEditor:tableScroll(table)
...
function FactionSignUpEditor:removeUI()
	self.super.removeUI(self)
end

function FactionSignUpEditor:onShow()
    self.super.onShow(self)

    self:refreshUI()
end

function FactionSignUpEditor:refreshUI()
...
function FactionSignUpEditor:refreshLeft()
    for i = 1, self.ZHENLIE_ROLE_COUNT do
...
function FactionSignUpEditor:refreshUp()
    self.img_zhenlie:setTexture(self.zhenlie_path[self.cur_zhenlie_index])
end

function FactionSignUpEditor:registerEvents()
    ...
    self.tabView:addMEListener(TFTABLEVIEW_SCROLL, handler(FactionSignUpEditor.tableScroll,self))
...
function FactionSignUpEditor.onDeleteClickHandle(sender)
...
function FactionSignUpEditor:runZhenlieAction(index)
...
function FactionSignUpEditor:insertToRight(toInsert)
    table.insert(self.memberData1, toInsert)
end

function FactionSignUpEditor.onOKClickHandle(sender)
    local self = sender.logic
    if FactionFightManager:CheckInSignUpTime() == false then
    ...
    TFDirector:send(c2s.UPDATE_APPLY, Msg)
end

function FactionSignUpEditor.onPageLeftClickHandle(sender)
    local self = sender.logic
...
function FactionSignUpEditor.onPageRightClickHandle(sender)
    local self = sender.logic
...
function FactionSignUpEditor.cellSizeForTable(table,idx)
    return 80, 230
end

function FactionSignUpEditor.numberOfCellsInTableView(table)
    local self = table.logic

    return #self.memberData1
end

function FactionSignUpEditor.tableCellAtIndex(table, idx)
    local self = table.logic;
...
function FactionSignUpEditor.cellTouchBeganHandle(cell)
...
function FactionSignUpEditor.cellTouchMovedHandle(cell)
    local self = cell.logic;
    self.isMove = true;
...
function FactionSignUpEditor.cellTouchEndedHandle(cell)
    local self = cell.logic;
...
function FactionSignUpEditor:insertToZhenlie(index)
    local temp = nil
    local toInsert = {}
...
function FactionSignUpEditor:deleteFromRight(index)
    table.remove(self.memberData1, index)
end

function FactionSignUpEditor:deleteFromLeft(index)
    table.remove(self.memberData[self.cur_zhenlie_index], index)
end

function FactionSignUpEditor:createSelectCussor(cell, pos)
    play_press();
...
function FactionSignUpEditor:moveSelectCussor(cell, pos)
...
function FactionSignUpEditor:hideFenge()
    for i = 1, self.FENGE_COUNT do
        self.fenge[i]:setVisible(false)
    end
    self.insert_index = nil
...
function FactionSignUpEditor:showFenge(index)
    self:hideFenge()
...
function FactionSignUpEditor:getClosestFenge(pos)
    local ins_list = {}
    for i = 1, self.FENGE_COUNT do
...
function FactionSignUpEditor:releaseSelectCussor(cell,pos)
    
end

function FactionSignUpEditor:removeLongTouchTimer()
...
function FactionSignUpEditor:updateMemberInfo()
    self.leaderData = {}
    self.memberData = {}
    --未报名帮派成员
    self.memberData1 = {}
...
function FactionSignUpEditor:dataReady()
    for i = 1, self.ZHENLIE_COUNT do        
        self.leaderData[i] = clone( FactionFightManager:getLeaderDataByIndex(i) )
        self.memberData[i] = clone( FactionFightManager:getMemberDataByIndex(i) )
    end
    self.memberData1 = FactionFightManager:getUnSignupMemberData()
    self.tabView:reloadData()

    self:refreshLeft()
end

function FactionSignUpEditor:removeEvents()
...
function FactionSignUpEditor:dispose()
	self.super.dispose(self)
end

return FactionSignUpEditor