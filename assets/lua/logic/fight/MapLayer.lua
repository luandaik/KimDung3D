...
--
MapEffectOffSetData = nil
if (GameConfig.HD) then
	MapEffectOffSetData = require("lua.table.t_s_map_effect_offset")
end
...
function MapLayer:AddMapEffect(bFrontEffect, fightBGImg, mapID, effectPos)
	...
	if bFrontEffect then
		...
		if (GameConfig.HD) then
			local item = MapEffectOffSetData:objectByID(mapID)
			if item then
				offset = ccp(item.offsetX,item.offsetY)
			end
		end
	end
	...
	effect:setPosition(ccp(uiSize.width/2 + offset.x, uiSize.height/2 + offset.y))
...
function MapLayer:GetMapID()
	...
	elseif fightType == 15 then --矿洞
		return 30
	elseif fightType == EnumFightType.JianFeng then
		if (GameConfig.Old_Jian) then
			return 41
		else
			return 1
		end
	elseif fightType == 25 then
		return WestClimbManager:getMapId()
	elseif fightType == 28 then
		return 32
	elseif fightType == EnumFightType.JapanIsland then
		return JapanIslandManager:getMapId()
	elseif fightType == EnumFightType.HeroConference then
		return 43
	elseif fightType == EnumFightType.HeroicBoss then
		return HeroListManager:getMapId()
	elseif fightType == EnumFightType.RankKing then
		if (GameConfig.Old_Jian) then
			return 40
		else
			return 1
