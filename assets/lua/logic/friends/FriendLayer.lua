    ...
    if layer.sideBtns[index] then
        ...
        if isFromeBtn then
            ...
            if index == localVars.pageIndex.friendAssist then
                offY = layer.tableViewPanel:getContentSize().height - 190*(#localVars.tableViewDataSource)
            else
...
function FriendLayer:initUI(ui)
    ...
    local verLock = VersionLockData:objectByID(EnumVersionLockType.AssistFight_Lock)
    if verLock and verLock.open == 0 then
        self.sideBtns[4]:setVisible(false)
    end
...
function FriendLayer:canGiveRole( playerId, role, needMsg)
    ...
    if cardRole and cardRole.gmId ~= 0 then
        ...
        if (cardRole.quality == 6 and useCount < 5) or
            (cardRole.quality == 5 and useCount < 3) or 
            (cardRole.quality < 5 and useCount < 1) then
        -- if (cardRole.quality == 5 and useCount < 3) or (cardRole.quality ~= 5 and useCount < 1) then
            return true
        else            
            if needMsg then
