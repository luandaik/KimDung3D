...
function ActivityOnline:initUI(ui)
    ...
    for i=1,rewardCount do
        ...
        timeText = OperationActivitiesManager:TimeConvertString(rewardData.id-temp_Time)
        temp_Time = rewardData.id
...
function ActivityOnline:removeEvents()
    ...
    if self.onlineRewardTimesList then
        for k,v in pairs(self.onlineRewardTimesList) do
            OperationActivitiesManager:removeOnlineRewardTimer(k)
        end
        self.onlineRewardTimesList = nil
    end
end
    ...
    for i=1,rewardCount do
        ...
        if widget.index <= self.OnlineRewardTimes  then
            ...
            if widget.index >= (self.OnlineRewardTimes + 1) then
                -- print("开启在线奖励定时器")
                self.onlineRewardTimesList = self.onlineRewardTimesList or {}
                OperationActivitiesManager:addOnlineRewardListener(widget, 3000+widget.index, ActivityOnline.onlineRewardUpdate)
                self.onlineRewardTimesList[3000+widget.index] = true
