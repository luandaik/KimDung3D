...
function Activity_scoreexchange:drawCell(node)
    ...
    if self.activity.type == OperationActivitiesManager.Type_Score_Exchange then
        -- 十连抽送积分 活动 特殊显示
        txt_desc:setText(localizable.activity_score_label)

        if rewardInfo.maxScore then--十连抽送积分活动 范围积分显示
            txt_score:setText( tostring(rewardInfo.maxScore) .. "-" .. tostring(rewardInfo.minScore) )
        else
