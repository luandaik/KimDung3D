...
function GoldEggItemResultLayer:setEggType(EggType)
	...
	local times = GoldEggManager:getMaxEggTimes(self.EggType)
	times = math.min(times,10)
	self.getTenCardBtn.times 	= times
	if times <= 1 then
		self.getTenCardBtn:setVisible(false)
		return
	end
	self.txt_cs:setText(times)
end
...
function GoldEggItemResultLayer:initUI(ui)
	...
	self.txt_cs 	= TFDirector:getChildByPath(self.getTenCardBtn, "txt_cs")
...
-- }
  ...
  --   end
	...
	self.txt_tips:setVisible(false)
	self.img_tdj:setVisible(false)
	if self.EggType == 3 then
		self:draw3Btn(true)
		if self.goodsInfo.isSpecial == 1 then
			self.txt_tips:setVisible(true)
			self.img_tdj:setVisible(true)
		end
	else
		self:drawBtn(true)
	end
