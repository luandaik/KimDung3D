...
function GoldEggMainLayer:initUI(ui)
    ...
    self.img_dan1    = TFDirector:getChildByPath(self, 'panel_dan1')
    self.img_dan2    = TFDirector:getChildByPath(self, 'panel_dan2')
    self.img_dan3    = TFDirector:getChildByPath(self, 'panel_dan3')
    ...
    self.txt_time1 = TFDirector:getChildByPath(self, 'txt_time1')
    self.txt_tip = TFDirector:getChildByPath(self, 'txt_tips')
    ...
    for v in GoldEggManager.showOtherHistory:iterator() do
        ...
        if v.rewardList then
            for i=1,#v.rewardList do
                ...
                local bg_icon = TFDirector:getChildByPath(item_panel, 'bg_icon')
                Public:loadIconNode(bg_icon,rewardItem)
                local img_tdj = TFDirector:getChildByPath(item_panel, 'img_tdj')
                layer:addChild(item_panel)
                if reward.isSpecial == 1 then
                    img_tdj:setVisible(true)
                else
                    img_tdj:setVisible(false)
                end
                item_panel:setPosition(ccp(width,4))
                width = width + item_panel:getContentSize().width
                local txt_num = TFDirector:getChildByPath(item_panel, 'txt_num')
                if txt_num then
...
function GoldEggMainLayer:registerEvents()
    ...
    local activity = OperationActivitiesManager:ActivityWithType(OperationActivitiesManager.Type_Hit_Egg,true)
    self.activity = activity

    if activity and activity.status == 2 then
    ...
    elseif activity then
        ...
        local descTime = self:getLeftTime(curTime, activity.endTime, activity.status)

        local descTime0 = ""--self:getLeftTime(curTime, activity.delayTime, activity.status)

        self.txt_time:setText(localizable.goldEggMain_time1 .. descTime)
        self.txt_time1:setText("") --localizable.goldEggMain_time2 .. descTime0

        if curTime > self.activity.endTime then
...
function GoldEggMainLayer:getLeftTime(time1,time2)
    local timeCount = time2 - time1
    if timeCount <= 0 then
        return '00:00:00'
    end

    local hour = math.floor(timeCount/3600)
    timeCount = timeCount - hour * 3600
    local min = math.floor(timeCount/60)
    timeCount = timeCount - min * 60
    local sec = math.floor(timeCount)
...
function GoldEggMainLayer:refreshUI()
    ...
    --     -- self.txt_time:setText(stringUtils.format(localizable.common_activity_time2,descTime))
    -- end
    -- print("activity.multiSever==",activity)
    self.isCrossServer = false
    local activity_score = OperationActivitiesManager:ActivityWithType(OperationActivitiesManager.Type_Score_Egg,true)
    ...
    self:drawEgg3(self.img_dan3)
end

function GoldEggMainLayer:showNextTimer()
    if self.activity == nil then
        self.activity = OperationActivitiesManager:ActivityWithType(OperationActivitiesManager.Type_Hit_Egg,true)
    end
    if self.activity == nil then
        if self.end_timerID then
            TFDirector:removeTimer(self.end_timerID)
    ...
    local curTime = MainPlayer:getNowtime()
    self.time = self.activity.endTime - curTime
    if self.time <=  0 then
        if self.end_timerID then
            TFDirector:removeTimer(self.end_timerID)
            self.end_timerID = nil
    ...
    local descTime = self:getLeftTime(curTime, self.activity.endTime, self.activity.status)

    local descTime0 = ""--self:getLeftTime(curTime, self.activity.delayTime, self.activity.status)
...
function GoldEggMainLayer:printEggInfo(eggInfo)
    -- body
    --local eggDesc = {"银蛋", "金蛋"}
    --print(GoldEggManager.EggInfoList)
...
function GoldEggMainLayer:drawEgg3(eggNode)
    ...
    local txt_curnum      = TFDirector:getChildByPath(eggNode, 'txt_num')
    local txt_own       = TFDirector:getChildByPath(eggNode, 'txt_yongyou')

    --local myOwn = "(当前拥有"..myToolNum..")"
    local count = GoldEggManager:getColorEggTimes()
    local myOwn =stringUtils.format(localizable.goldEggMain_number0, count)
    txt_curnum:setText(myOwn)
    local myOwn =stringUtils.format(localizable.goldEggMain_number1, eggInfo.score)
    ...
    btn_zadan:addMEListener(TFWIDGET_CLICK, audioClickfun(
        function ()
            local count = GoldEggManager:getColorEggTimes()
            if count <= 0 then
                toastMessage(localizable.goldEggMain_can_hammer)
                return
            end
...
function GoldEggMainLayer:drawEgg(eggNode)
    ...
    btn_zadan:addMEListener(TFWIDGET_CLICK, audioClickfun(
        function ()
            if eggInfo.freeTime == 0 and myToolNum < commonReward.number then
                ...
                if eggType == 1 then
                    if MallManager:checkShopOneKey(commonReward.itemId) == true then
                        return
                    end
                end
    ...
    -- if CC_TARGET_PLATFORM == CC_PLATFORM_IOS and self.changePos[eggType] ~= true then
    --     txt_danhuanum:setFontSize(16)
    --     local posY = txt_danhuanum:getPositionY()
    --     txt_danhuanum:setPositionY(posY-5)
    --     self.changePos[eggType] = true
    -- end
