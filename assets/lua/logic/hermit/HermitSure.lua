...
function HermitSure:refreshUI()
    ...
    local length = math.min(6 , rewardList:length())
    self.length = length
...
function HermitSure.cellSizeForTable(table,idx)
    return 160,138*table.logic.length+80
end
    ...
    if nil == cell then
        ...
        for i=1,6 do
            ...
            if index <= rewardList:length() then
                ...
                reward_item:setPosition(138*i - 130,0)
...
function HermitSure.numberOfCellsInTableView(table)
    local length = math.ceil(table.logic.rewardList:length()/6)
    return length
end
