...
function RoleReBirthLayer:calculateReward()
    ...
    if cardRole == nil then
        ...
        kuXingCost, zhengNianCost       = cardRole:getMeridianCostInfo()
        kuXingCost                      = math.floor(kuXingCost * RecycleSkillCost / 100)
        zhengNianCost                   = math.floor(zhengNianCost * RecycleSkillCost / 100)
    ...
    if kuXingCost > 0 then
        local rewardInfo  = {}
        rewardInfo.type   = EnumDropType.GOODS
        rewardInfo.number = kuXingCost
        rewardInfo.itemId = 30125
        local _rewardInfo = BaseDataManager:getReward(rewardInfo)
        calculateRewardList:push(_rewardInfo);
    end
    if zhengNianCost > 0 then
        local rewardInfo  = {}
        rewardInfo.type   = EnumDropType.GOODS
        rewardInfo.number = zhengNianCost
        rewardInfo.itemId = 30286
        local _rewardInfo = BaseDataManager:getReward(rewardInfo)
        calculateRewardList:push(_rewardInfo);
    end
