...
function IllustrationEquipLayer.touchRoleNode(sender)
    ...
    local info          = self.qualityEquipList[quality][index]
    local id            = info.id
    local suit_id       = EquipmentTemplateData:objectByID(id).suit_id
    -- local equipMentData = ItemData:objectByID(id)
    -- --detail
    -- print("我点击了- --", equipMentData.name)
    local layer = nil
    if suit_id == 0 then
        layer  = require("lua.logic.illustration.IllustrationEquipDetailLayer"):new(id)
    else
        layer  = require("lua.logic.illustration.IllustrationTaoZhuangLayer"):new(id)
    end
