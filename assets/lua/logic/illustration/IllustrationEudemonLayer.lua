...
local IllustrationEudemonLayer = class("IllustrationEudemonLayer", BaseLayer)

function IllustrationEudemonLayer:ctor()
    self.super.ctor(self)
    self:init("lua.uiconfig_mango_new.handbook.HandbookEquipLayer")
end

function IllustrationEudemonLayer:initUI(ui)
    ...
    --
    self.QualityRes = {}
    self.QualityRes[6] = "ui_new/handbook/pet_6.png"
    self.QualityRes[5] = "ui_new/handbook/pet_5.png"
    self.QualityRes[4] = "ui_new/handbook/pet_4.png"
    self.QualityRes[3] = "ui_new/handbook/pet_3.png"
    self.QualityRes[2] = "ui_new/handbook/pet_2.png"
    self.QualityRes[1] = "ui_new/handbook/pet_1.png"
...
function IllustrationEudemonLayer:removeUI()
    self.super.removeUI(self)
end

function IllustrationEudemonLayer:registerEvents()
    self.super.registerEvents(self)

    self.btn_all:addMEListener(TFWIDGET_CLICK, audioClickfun(self.BtnClickHandle))
end

function IllustrationEudemonLayer:removeEvents()
    --按钮事件
...
-- index 对应职业 0为全部
function IllustrationEudemonLayer:FilterRoleListData(kind)
	self.qualityZhenfaList = {}

    self.qualityZhenfaList = EudemonManager:getAllEudemonQualityList()
    self.qualityNumList  = EudemonManager:CountNumInList(self.qualityZhenfaList)

    self.QualityNum     = #self.qualityNumList


    for i=1,(EudemonManager.MAX_QUALITY) do
        if self.qualityZhenfaList[i] then
            table.sort(self.qualityZhenfaList[i],sortlist)
        end
    end
...
function IllustrationEudemonLayer:drawCellTitle(cell, QualityIndex, curNum, totalNum)
...
function IllustrationEudemonLayer:drawRole(node, quality, index)
	if node == nil or self.qualityZhenfaList == nil then
		return
	end
	--空间
	...
	local info 			= self.qualityZhenfaList[quality][index]
	local id 			= info.id
...
function IllustrationEudemonLayer:drawCellDetail(cell, quality, index, num)
	-- 绘制背景
    local rowTotal = math.ceil(num/5)
...
function IllustrationEudemonLayer:drawCellWithIndex(cell, cellIndex)
    -- 对cellindex区域
...
function IllustrationEudemonLayer:drawRoleList()
    ...
    tableView:addMEListener(TFTABLEVIEW_SIZEFORINDEX, IllustrationEudemonLayer.cellSizeForTable)
    tableView:addMEListener(TFTABLEVIEW_SIZEATINDEX, IllustrationEudemonLayer.tableCellAtIndex)
    tableView:addMEListener(TFTABLEVIEW_NUMOFCELLSINTABLEVIEW, IllustrationEudemonLayer.numberOfCellsInTableView)
...
function IllustrationEudemonLayer.numberOfCellsInTableView(table)
	local self = table.logic
    return self.TableCellNum
end

function IllustrationEudemonLayer.cellSizeForTable(table,idx)
    local self = table.logic
...
function IllustrationEudemonLayer.tableCellAtIndex(table, idx)
...
function IllustrationEudemonLayer:drawDefault(index)
...
function IllustrationEudemonLayer.BtnClickHandle(sender)
...
function IllustrationEudemonLayer.touchRoleNode(sender)
    local self      = sender.logic
    local index     = sender.index
    ...
    local equipMentData = ItemData:objectByID(id)
    -- --detail
     print(info.typeId,"我点击了- --", equipMentData.name)

    EudemonManager:OpemEudemonMainLayer(info.typeId, EnumFightStrategyType.StrategyType_PVE, true, id)
    -- local layer  = require("lua.logic.illustration.IllustrationZhenfaDetailLayer"):new(id)
...
return IllustrationEudemonLayer