...
local IllustrationMiYaoLayer = class("IllustrationMiYaoLayer", BaseLayer)

function IllustrationMiYaoLayer:ctor()
    self.super.ctor(self)
    self:init("lua.uiconfig_mango_new.handbook.HandbookEquipLayer")
end

function IllustrationMiYaoLayer:initUI(ui)
...
function IllustrationMiYaoLayer:removeUI()
    self.super.removeUI(self)
end

function IllustrationMiYaoLayer:registerEvents()
    self.super.registerEvents(self)

    self.btn_all:addMEListener(TFWIDGET_CLICK, audioClickfun(self.BtnClickHandle))
end

function IllustrationMiYaoLayer:removeEvents()
    --按钮事件
...
-- index 对应职业 0为全部
function IllustrationMiYaoLayer:FilterRoleListData(kind)
	self.qualityZhenfaList = {}

    self.qualityZhenfaList = IllustrationManager:FilterMiYaoList(kind)
...
function IllustrationMiYaoLayer:drawCellTitle(cell, QualityIndex, curNum, totalNum)
...
function IllustrationMiYaoLayer:drawRole(node, quality, index)
...
function IllustrationMiYaoLayer:drawCellDetail(cell, quality, index, num)
	-- 绘制背景
    local rowTotal = math.ceil(num/5)
...
function IllustrationMiYaoLayer:drawCellWithIndex(cell, cellIndex)
    -- 对cellindex区域
...
function IllustrationMiYaoLayer:drawRoleList()
    ...
    tableView:addMEListener(TFTABLEVIEW_SIZEFORINDEX, IllustrationMiYaoLayer.cellSizeForTable)
    tableView:addMEListener(TFTABLEVIEW_SIZEATINDEX, IllustrationMiYaoLayer.tableCellAtIndex)
    tableView:addMEListener(TFTABLEVIEW_NUMOFCELLSINTABLEVIEW, IllustrationMiYaoLayer.numberOfCellsInTableView)
...
function IllustrationMiYaoLayer.numberOfCellsInTableView(table)
	local self = table.logic
    return self.TableCellNum
end

function IllustrationMiYaoLayer.cellSizeForTable(table,idx)
    local self = table.logic
...
function IllustrationMiYaoLayer.tableCellAtIndex(table, idx)
...
function IllustrationMiYaoLayer:drawDefault(index)
...
function IllustrationMiYaoLayer.BtnClickHandle(sender)
...
function IllustrationMiYaoLayer.touchRoleNode(sender)
    local self      = sender.logic
    local index     = sender.index
    ...
    local layer  = require("lua.logic.illustration.IllustrationMiYaoDetailLayer"):new(id)
    AlertManager:addLayer(layer, AlertManager.BLOCK_AND_GRAY_CLOSE)
    AlertManager:show()
    
end
return IllustrationMiYaoLayer