...
function IllustrationEudemonLayer:initUI(ui)
    ...
    --
    self.QualityRes = {}
    self.QualityRes[5] = "ui_new/handbook/tianshu_3.png"
    self.QualityRes[4] = "ui_new/handbook/xf_2.png"
    self.QualityRes[3] = "ui_new/handbook/xf_3.png"
...
function IllustrationEudemonLayer:FilterRoleListData(kind)
	self.qualityXinfaList = {}

    self.qualityXinfaLis = IllustrationManager:getXinFaList()
    self.qualityNumList  = IllustrationManager:CountNumInList(self.qualityXinfaLis)

    self.QualityNum     = #self.qualityNumList

    --心法品质从3开始算
    for i=3,(XinFaManager.MAX_QUALITY) do
        if self.qualityXinfaLis[i] then
            table.sort(self.qualityXinfaLis[i],sortlist)
        end
    end
...
function IllustrationEudemonLayer:drawRole(node, quality, index)
	if node == nil or self.qualityXinfaLis == nil then
		return
	end
	--空间
	...
	local info 			= self.qualityXinfaLis[quality][index]
	local id 			= info.id
...
function IllustrationEudemonLayer.touchRoleNode(sender)
    ...
    local equipMentData = ItemData:objectByID(id)
    -- --detail
     print(info.typeId,"我点击了- --", equipMentData.name,id)

    local layer = nil
    
    layer  = require("lua.logic.illustration.IllustrationXinFaDetailLayer"):new(id)
