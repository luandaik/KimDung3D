-- @Author: crb
-- @Date:   2018-04-09 15:00:27
-- @Last Modified by:   cc
-- @Last Modified time: 2018-04-10 11:51:38
...
function ShoppingLayer:ctor( tGoodsInfo, bCanBuy )
    self.super.ctor(self)
    self.tGoodsInfo = tGoodsInfo
...
function ShoppingLayer:initUI(ui)
	...
	self.slider_shop:setGrayEnabled(true)
	self.btn_add:setGrayEnabled(true)
	self.btn_reduce:setGrayEnabled(true)
...
function ShoppingLayer:setOldPrice( id , num )
	ItemData:objectByID(self.commodityData.itemid)
...
function ShoppingLayer:refreshUI()
	local item = ItemData:objectByID(self.tGoodsInfo.itemId)
	if item == nil  then
	...
	Public:addPieceImg(self.img_icon,{type = EnumDropType.GOODS,itemid = self.tGoodsInfo.itemId});
	...
	-- 	self.txt_remaining_num:setVisible(true)
	-- 	self.lbl_remaining_title:setVisible(true)
	-- 	self.txt_remaining_num:setText(ShopData:objectByID(self.id).number)
	-- 	self.txt_number:setText(ShopData:objectByID(self.id).number)
	-- 	self.lbl_remaining_title:setText(localizable.activity_OnceShop)
	-- 	self.now_txt_price:setText(shop.consume_number)
	-- 	self.old_txt_price:setText(shop.consume_number)
	-- end

	self.old_txt_price:setText(self.tGoodsInfo.nPrice)
	self.now_txt_price:setText(self.tGoodsInfo.nDiscount / 10 * self.tGoodsInfo.nPrice)
	...
	self.txt_num:setText(self.tGoodsInfo.nCount)
...
function ShoppingLayer:registerEvents()
	...
	self.btn_buy:addMEListener(TFWIDGET_CLICK, function (  )
		local tActivityInfo = OperationActivitiesManager:getActivityInfoByType(OperationActivitiesManager.Type_JiangHuJuHui)
		OperationActivitiesManager:sendMsgToGetActivityReward(tActivityInfo.id, self.tGoodsInfo.index, nil, nil, nil, self.tGoodsInfo.nCount)
