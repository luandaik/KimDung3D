...
function Leaderboard:initUI( ui )
    ...
    self.BtnTab[RankListType.Rank_List_Xiling] 			= TFDirector:getChildByPath(ui, "btn_xlb")
	self.BtnTab[RankListType.Rank_List_CrossZhengBa] 	= TFDirector:getChildByPath(ui, "btn_tjb")
	if CrossZhengBaManager:getRankServerSwitchStatue() == true then
    	self.BtnTab[RankListType.Rank_List_CrossZhengBa]:setVisible(true)
    else
    	self.BtnTab[RankListType.Rank_List_CrossZhengBa]:setVisible(false)
    end
    local verLock = VersionLockData:objectByID(EnumVersionLockType.Adventure_Lock)
    -- if verLock and verLock.open == 0 then
    if 1 then -- 网红说永久隐藏掉。。。
        self.BtnTab[13]:setVisible(false)
        local pos 	= self.BtnTab[13]:getPosition()
		local pos1 	= self.BtnTab[RankListType.Rank_List_Xiling]:getPosition()
        self.BtnTab[RankListType.Rank_List_Xiling]:setPosition(pos)
        self.BtnTab[RankListType.Rank_List_CrossZhengBa]:setPosition(pos1)
    end

    verLock = VersionLockData:objectByID(EnumVersionLockType.WestClimb_Lock)
    if verLock and verLock.open == 0 then
        self.BtnTab[RankListType.Rank_List_Xiling]:setVisible(false)
    end
    ...
    self.btn_normalTextures[RankListType.Rank_List_Xiling] = 'ui_new/leaderboard/btn_em.png'
    self.btn_normalTextures[RankListType.Rank_List_CrossZhengBa] = 'ui_new/leaderboard/btn_tj.png'
    ...
    self.btn_selectedTextures[RankListType.Rank_List_Xiling] = 'ui_new/leaderboard/btn_em_hl.png'
    self.btn_selectedTextures[RankListType.Rank_List_CrossZhengBa] = 'ui_new/leaderboard/btn_tj_hl.png'
    --关闭神兵榜
	...
	-- self.Panel_sv:setInnerContainerSize(CCSizeMake(1238, self.Panel_sv:getSize().height))
end
    ...
    if cell.panel then
    	...
    	if cell.index == self.cell_curr_max and self.needViewMoreBtn then
    		...
    		if self.isFristIn == false then
    			...
    			if item and item.ranking == nil then
    				item.ranking = cell.index
    			end
...
function Leaderboard:CreateTableCell(cell)
	...
	if cell.type == nil then
        ...
        elseif self.btn_curr_type == RankListType.Rank_List_CrossZhengBa then
        	panel = require('lua.logic.leaderboard.CrossZhengBacell'):new()
    	else
...
function Leaderboard:getRankDataMap(userData)
	...
	if self.btn_curr_type == RankListType.Rank_List_CrossZhengBa then
	...
	self.currRankObj:showDetails(self.cellDataMap.rankInfo[self.cell_select_index])
	if self.btn_curr_type == RankListType.Rank_List_ShaLu or self.btn_curr_type == RankListType.Rank_List_Xiling or self.btn_curr_type == RankListType.Rank_List_CrossZhengBa then
		self.txtDianzancishu:setVisible(false)
    	self.txtDZCS:setVisible(false)
    	self.txtMyzan:setVisible(false)
    else
