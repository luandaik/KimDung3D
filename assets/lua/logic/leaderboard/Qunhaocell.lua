...
function Qunhaocell:initUI( ui )
	...
	self.txt_server = TFDirector:getChildByPath(ui, "txt_server")
...
function Qunhaocell:SetData(layer,item)
	...
	if item == nil then
		...
		if layer.btn_curr_type == RankListType.Rank_List_Xiling then
			self.txt_zan:setVisible(false)
		else
			self.txt_zan:setVisible(true)
		...
		elseif layer.btn_curr_type == RankListType.Rank_List_Xiling then
			self.txt_name:setString(item.playerName)
			self.txt_zhandouli:setVisible(false)
			self.txt_zhandouli_word:setVisible(false)
			self.txt_cengshu:setVisible(true)
			--self.txt_cengshu:setString(item.value.."层")
			self.txt_cengshu:setString(stringUtils.format(localizable.common_ceng,item.floorId))
			item.ranking = item.rank

			if item.ranking == layer.cell_select_index then
				self.bg_select:setVisible(true)
			end

			self.Btn_zan:setVisible(false)
		...
		self.Btn_huifang.playerId = item.playerId
		if item.floorId and item.floorId >= 5 then
...
function Qunhaocell.ReplayeClick( btn )
	print('btn.playerId = ',btn.playerId)
	if btn and (btn.playerId and btn.playerId ~= 0) then
		showLoading()
        TFDirector:send(c2s.SERDANG_REPLAY_REQUEST, {btn.playerId})
        
	else
		print('cannot find replay id = ',btn.playerId)
	end
end
return Qunhaocell