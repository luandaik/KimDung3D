...
MainLevelUpLayer.CONST_JIAYUAN = ConstantData:objectByID("ShareWayLevelupOpen").value or 21
...
function MainLevelUpLayer:initUI(ui)
    ...
    self.pic_role         = TFDirector:getChildByPath(self.panel_role, 'img_role')
    ...
    self.btn_queding = TFDirector:getChildByPath(self.panel_button, "btn_queding")
    self.btn_queding.logic = self
    self.btn_fenxiang = TFDirector:getChildByPath(self.panel_button, "btn_fx")
...
--家园分享,条件显示
function MainLevelUpLayer:checkBtns()
    local shareOpen = false
    if shareOpen and MainPlayer:getLevel() >= self.CONST_JIAYUAN and MainPlayer:getServerSwitchStatue(ServerSwitchType.JiaYuan) then
        
        self.btn_close:setVisible(false)
        
        self.panel_button:setVisible(true)
        self.panel_button:setOpacity(0)
        self.panel_button:runAction(CCFadeIn:create(0.8))
    else
        self.panel_button:setVisible(false)
        self.btn_close:setVisible(true)
        self.btn_close:setOpacity(0)
        self.btn_close:runAction(CCFadeIn:create(0.8))
    end
end
...
function MainLevelUpLayer:refreshUI()
    ...
    self.ui:setAnimationCallBack("put_role", TFANIMATION_END, function()
        ...
        resPath = "effect/ui/level_up_light.xml"
        TFResourceHelper:instance():addArmatureFromJsonFile(resPath)
...
function MainLevelUpLayer:registerEvents()
    ...
    self.btn_queding:addMEListener(TFWIDGET_CLICK, 
    audioClickfun(function() 
        AlertManager:close()
        PlayerGuideManager:OnMainLevelUpLayerClose()
    end),1)

    self.btn_fenxiang:addMEListener(TFWIDGET_CLICK,
    audioClickfun(function() 
        QiyuManager:goShare(2)
    end),1)
