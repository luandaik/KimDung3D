...
function AdventureShoppingLayer:initUI(ui)
	...
	self.old_price_bg:setVisible(true)
...
function AdventureShoppingLayer:refreshUI()
	...
	self.old_img_res_icon:setTexture(GetResourceIconForGeneralHead(self.type))

	local maxNum = 0
	local titleStr = ""
	if (self.type == HeadResType.VIPJF) then
		maxNum = self.commodityData.purchaseNum
		titleStr = localizable.activity_limitShop
	else
		maxNum = BagManager:getItemNumById(self.commodityData.consume_id)
		titleStr = localizable.youli_shop_txt2
	end
	self.txt_remaining_num:setText(maxNum)
	self.lbl_remaining_title:setText(titleStr) 
	self:setShopNum(maxNum or 1)
	
	local remainingNumber = self:getRemainingNumber()
	local percent = math.ceil(maxNum/remainingNumber*100)
	...
	-- 折扣 10为原价
	if (self.commodityData.discount and self.commodityData.discount ~= 10) then
		self.img_zhekou:setVisible(true)
		self.img_zhekou:setTexture("ui_new/operatingactivities/z"..self.commodityData.discount..".png")
	end
end
...
function AdventureShoppingLayer:getRemainingNumber()
	if (self.type == HeadResType.VIPJF) then
		return self.commodityData.purchaseNum
	end
...
function AdventureShoppingLayer:setShopNum( num )
	...
	local currentResValue = BagManager:getItemNumById(self.commodityData.consume_id)
	if (self.type == HeadResType.VIPJF) then
		currentResValue = PayManager:getVipIntegral()
	end
	if currentResValue < singlePrice * num then
		...
		self.old_txt_price:setColor(ccc3(255, 255, 255))
	end
...
function AdventureShoppingLayer.buyButtonClickHandle(sender)
	...
	if currentResValue < num * singlePrice then 
		local itemName = ""
		if (self.type == HeadResType.VIPJF) then
			itemName = localizable.VipShop_Cost_Name
		else
			itemName = ItemData:objectByID(self.commodityData.consume_id).name
		end
		toastMessage(itemName..localizable.youli_shop_txt1)
		return
	end
	...
	local hasCoin =  BagManager:getItemNumById(self.commodityData.consume_id)
	if (self.type == HeadResType.VIPJF) then
		hasCoin = PayManager:getVipIntegral()
	end
	if totalPrice <= hasCoin then
		showLoading()
		if (self.type == HeadResType.VIPJF) then
			PayManager:sendVipShoppingData(self.commodityData.id, num)
