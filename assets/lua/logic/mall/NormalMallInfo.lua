...
function NormalMallInfo:initUI(ui,layer)
    ...
    self.btnNormalImg = {
        ...
        'ui_new/shop/btn_putong_unselected.png',--新增
    ...
    self.btnHoldImg = {
        ...
        'ui_new/shop/btn_putong_selected.png',--新增
...
function NormalMallInfo:show(index)
    ...
    self.logic.btnList[1]:setPositionY(523)
    self:refreshButton()
    self:select(index)
end
...
function NormalMallInfo:refreshButton()
    ...
    local list = MallManager:getFixedStoreCommodityList(1)
    ...
    if randomStore[RandomStoreType.Zhenbao]:isOpen() == false then
        self.logic.btnList[5]:setVisible(false)
    end
    local temp = 0
    ...
    for k,v in ipairs(self.logic.btnList) do
        if v:isVisible() then
            temp = temp + 1
            v:setPositionY( pos_y - (temp-1)*100 )            
        end
    end
...
function NormalMallInfo:select(index)
    print('NormalMallInfo-----',index)
    self.selectedIndex = index
    if index then
        if index == 1 then
            local list = MallManager:getFixedStoreCommodityList(1)
            if list:length() > 0 then
                self:selectTimeLimitShop()
            else
...
function NormalMallInfo:selectTimeLimitShop()
    self:hideCurrentLayer()

    --创建显示内容图层
    if self.giftsTimeLimitPage == nil then
        self.giftsTimeLimitPage = require('lua.logic.mall.GiftsShopTimeLimitPage'):new()
        self.panel_list:addChild(self.giftsTimeLimitPage)
    end
    
    self.currentLayer = self.giftsTimeLimitPage
    self:showCurrentLayer()
end
...
function NormalMallInfo:dispose()
    ...
    if self.giftsTimeLimitPage then
        self.giftsTimeLimitPage:dispose()
        self.giftsTimeLimitPage = nil
    end
