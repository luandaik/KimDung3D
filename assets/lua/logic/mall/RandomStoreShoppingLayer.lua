...
function RandomStoreShoppingLayer:initUI(ui)
	...
	self.txt_desc = Public:createRichtextHB(txt_desc, "", EnumRichtextType.ShopOrUse)
end
...
function RandomStoreShoppingLayer:refreshUI()
	...
	self.txt_desc:setText(stringUtils.richText(goodsData.details))
	...
	self.old_img_res_icon:setTexture(GetResourceIcon(shopEntry.res_type))
	...
	elseif goodsData.type == 1 then
		self.txt_own_num:setVisible(true)		
		self.txt_own_num:setText(EquipmentManager:getEquipNumberByID(goodsData.id))
	elseif goodsData.type == 36 then
		self.txt_own_num:setVisible(true)		
		self.txt_own_num:setText(XinFaManager:getXinFaPieceNumById(goodsData.id))
	elseif goodsData.type == 35 then
		self.txt_own_num:setVisible(true)		
		self.txt_own_num:setText(XinFaManager:getIdenticalXinFaMaxNumById(goodsData.id))
	else
		self.txt_own_num:setText(item_num)
	end
...
function RandomStoreShoppingLayer.reduceButtonClickHandle(sender)
	...
	local maxNum = sender.logic.commodityData:getNumber()
	if (sender.logic.commodityData.template.id == 30425) then
		maxNum = 99
	end
	local percent = math.ceil(count/maxNum*100)
...
function RandomStoreShoppingLayer:setShopNum( num )
	...
	-- if (self.commodityData.template.id == 30425) then
	-- 	num = 99
	-- end
...
function RandomStoreShoppingLayer.buyButtonClickHandle(sender)
	...
	local shopEntry = self.commodityData:getShopEntry()
	local totalPrice = shopEntry.res_num * num
	if (EnumDropType.XIAYI == shopEntry.res_type) then
		local xiaYICount = MainPlayer:getResValueByType(EnumDropType.XIAYI)
		if (xiaYICount < totalPrice) then
			if (MallManager:showBuyXiaYiItem()) then
				return
			end
		end
	end
...
function RandomStoreShoppingLayer.sliderTouchEndHandle(sender)
	local self = sender.logic
	local maxNum = self.commodityData:getNumber()
	if (self.commodityData.template.id == 30425) then
		maxNum = 99
	end
	local percent = math.ceil(self.num/maxNum*100)
	self.slider_shop:setPercent(percent)
	self.bg_jindushuzhi:setVisible(false)
end
...
function RandomStoreShoppingLayer:freshSliderNum()
	...
	local num = math.ceil(percent*maxNum)
	num = math.max(num,1)
	self:setShopNum(num)
