...
function ShopItemCell:setData( data , log)
	self.id = data.id
	...
	if log == true and data.template.id ~= 30425 then  
		if self.txt_name.textLabel then
			self.txt_name.textLabel:removeFromParent()
			self.txt_name.textLabel = nil
		end
    	local textLabel = TFScrollRichText:create(name)
    	textLabel:setSpecial(true)
    	textLabel:setTextPos(ccp(-50, -30))
    	textLabel:setZOrder(10)
    	textLabel:setPosition(ccp(-50, -30)) 
    	textLabel:setSize(CCSizeMake(90, 50))
    	textLabel:setSpeed(0.3)
    	textLabel:setScale(1.2)
    	textLabel:setColor(ccc3(0,0,0))
    	textLabel:setChangeFunc(ShopItemCell, ShopItemCell.OnShowEnd)
    	self.txt_name:addChild(textLabel)
    	self.txt_name.textLabel = textLabel
...
function ShopItemCell:OnShowEnd()
