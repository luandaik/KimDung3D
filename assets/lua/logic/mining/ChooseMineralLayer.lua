...
function ChooseMineralLayer:initUI(ui)
    ...
    --刷矿令
    self.img_res_bg2 = TFDirector:getChildByPath(ui, "img_res_bg2")
...
function ChooseMineralLayer:showUIData()
    ...
    elseif numInBag > 0 then
        self.txt_freetime:setVisible(false)
        self.img_res_bg:setVisible(false)
        self.img_res_bg2:setVisible(true)
        local txt_shuliang = TFDirector:getChildByPath(self.img_res_bg2, "txt_shuliang")
        txt_shuliang:setText(stringUtils.format(localizable.MiningLayer_text2,numInBag))
        local txt_price = TFDirector:getChildByPath(self.img_res_bg2, "txt_price")
        txt_price:setText(1)
    else
        self.txt_freetime:setVisible(false)
...
function ChooseMineralLayer:addCheckedEffect()
    ...
    self.checkEffect:setPosition(ccp(323, -110))
