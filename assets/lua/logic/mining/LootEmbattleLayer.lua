...
function LootEmbattleLayer:initUI(ui)
    ...
    for k=1,3 do
        ...
        for i=1,9 do
            ...
            local img_zf = TFDirector:getChildByPath(panel, "img_zf" .. i)
...
function LootEmbattleLayer:registerEvents()
    ...
    if self.generalHead then
        ...
        local closeFunc = function(sender)
            ...
            self.onCloseCallBack(self.btn_close)
        end
...
function LootEmbattleLayer.onCloseCallBack(sender)
    ...
    elseif self.armyRoleStatu[1] == 1 and self.armyRoleStatu[2] == 0 then
        ...
        if self.generalHead then
            self.generalHead.returnButtonClick(self.generalHead.btn_return)
        else
            AlertManager:close()
        end
        local info = MiningManager:getLootPlayerIndexAndMine()
...
function LootEmbattleLayer:showArmyUIData()    
    for k=1,2 do
        ...
        if k == 1 then
            ...
            local serverId = armyInfo.serverId
            serverId = math.mod(serverId,1000)
            name:setText("S" .. serverId .. armyInfo.details.name)
            ...
            Public:addInfoListen(img_headIcon,true,5,armyInfo.details.playerId,serverId)
        elseif k == 2 then
