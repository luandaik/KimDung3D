...
function LootMineralItem.onChooseRoleCallBack(sender)
    ...
    local minePlayerId  = mineList.minePlayerId
...
function LootMineralItem:showUIData()
    ...
    for k=1,6 do
        ...
        if next(self.mineList) ~= nil and self.mineList[k] ~= nil then
            local roleIcon = RoleData:objectByID(self.mineList[k].mineIcon)                             --pck change head icon and head icon frame
            if roleIcon == nil then
                roleIcon = RoleData:objectByID(77)
            end
            panel_role:setVisible(true)
            local serverId = self.mineList[k].serverId
            serverId = math.mod(serverId,1000)
            txt_name:setText("S" .. serverId .. self.mineList[k].playerName)
            txt_zhandouli:setText(self.mineList[k].power)
            img_role:setTexture(roleIcon:getIconPath()) 
            Public:addFrameImg(img_role,self.mineList[k].mineHeadIcon)                             --end
            ...
            local guardPlayerId = self.mineList[k].guardPlayerId
            if guardPlayerId == nil then
                bg_huwei:setVisible(false)
            else
                bg_huwei:setVisible(true)
                local huweiIcon = RoleData:objectByID(self.mineList[k].guardIcon)                               --pck change head icon and head icon frame
                if huweiIcon == nil then
                    huweiIcon = RoleData:objectByID(77)
                end
                img_huwei:setTexture(huweiIcon:getIconPath())
                Public:addFrameImg(img_huwei,self.mineList[k].guardHeadIcon)                               --end
