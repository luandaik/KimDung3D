...
function MiningArmyLayer:refreshUI()
    ...
    if not self.table_select then
        ...
        MiningManager:updateRoleList(self.fightType - EnumFightStrategyType.StrategyType_MINE1_DEF + 1)
        local miningRoleList = clone(MiningManager.cardRoleList)
        self.roleList = ZhengbaManager:getSortStrategy(self.fightType, self.currSortType, miningRoleList)
...
function MiningArmyLayer.onSkillBtnClick( btn )
