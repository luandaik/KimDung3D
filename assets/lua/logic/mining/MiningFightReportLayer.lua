...
function MiningFightReportLayer:drawTableview()
    ...
    FriendsTableView:setPosition( ccp( self.Panel_FriendList:getPositionX(), self.Panel_FriendList:getPositionY()) )
...
function MiningFightReportLayer:drawCell(node)
    ...
    if role3 then
        ...
        local serverId = math.mod(role3.serverId,1000)
        strExtend = stringUtils.format(localizable.Mining_UI3, "S" .. serverId .. role3.name, replayData.challengeGuardCount)
    end
    ...
    local serverId1 = math.mod(role1.serverId,1000)
    local serverId2 = math.mod(role2.serverId,1000)

    str = stringUtils.format(str,"S" .. serverId1 .. role1.name,"S" .. serverId2 .. role2.name, replayData.challengePlayerCount, strExtend)
...
function MiningFightReportLayer:drawPlayNode(node, player)
    ...
    local serverId = math.mod(player.serverId,1000) 

    txt_name:setText("S" .. serverId .. player.name)
    ...
    Public:addFrameImg(img_head,player.headPicFrame) 
    Public:addInfoListen(img_head,true,5,player.playerId,player.serverId)                      --end
