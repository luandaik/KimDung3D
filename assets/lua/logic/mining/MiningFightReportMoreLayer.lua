...
function MiningFightReportMoreLayer:drawCell(node)
    ...
    local serverId1 = math.mod(report.fromRole.serverId,1000)
    local serverId2 = math.mod(report.targetRole.serverId,1000)
    -- --%s与%s战斗，战斗失败
    local str = stringUtils.format(strExtend,"S" .. serverId1 .. report.fromRole.name,"S" .. serverId2 .. report.targetRole.name)
...
function MiningFightReportMoreLayer:drawPlayNode(node, player,reportId)
    ...
    Public:addInfoListen(img_head,true,5,player.playerId,player.serverId)
...
function MiningFightReportMoreLayer.onClickFightReport(sender)
    ...
    print("onclick fight replayId = ",MiningManager.replayId)

    showLoading()
    TFDirector:send(c2s.GAIN_BATTLE_VIDEO, {MiningManager.replayId,sender.reportId})
end
