...
function MiningProtectRecordLayer:drawRecordCell(node)
    ...
    if data.extraBrokerrage == 0 then
        ...
        local serverId = math.mod(data.robServerId,1000)
        -- str = TFLanguageManager:getString(ErrorCodeData.Mining_Protect_Record2)
        str = stringUtils.format(localizable.Mining_Protect_Record2, data.employerPlayerName, "S" .. serverId .. data.robPlayerName, data.extraBrokerrage)
    end
