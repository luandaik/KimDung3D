...
function MissionSkipNewLayer:loadData(mapid,difficulty)
    ...
    local map = MissionManager:getMapById(self.difficulty, self.mapid);
    ...
    --next info
    local nextMap = MissionManager:getMapById(self.difficulty,self.mapid+1);
...
function MissionSkipNewLayer:setBtnHandle(goonhandle, getstarhandle)
    ...
    if self.Btn_goon then
        ...
        self.Btn_goon:addMEListener(TFWIDGET_CLICK, audioClickfun(function()
            ...
            print('nononono')
            if (PlayerGuideMgr.now_step == 233) then
                --埋点id:5 点击继续闯关按钮
                TFDirector:send(c2s.DATA_COLLECT_REQUEST, {5})  
            end
            AlertManager:close(AlertManager.TWEEN_NONE)  
            goonhandle()                      
        end),1)
...
function MissionSkipNewLayer.onOpenBoxClickHandle( sender )
    ...
    --if true then
        ...
        --埋点id:2.3.4   点击领取关卡宝箱
        local PlayerGuideMgr = require("lua.gamedata.PlayerGuideManager")
        if (PlayerGuideMgr.now_step >= 230 and PlayerGuideMgr.now_step <= 232) then
            local id = PlayerGuideMgr.now_step - 228
            TFDirector:send(c2s.DATA_COLLECT_REQUEST, {id})
        end
