...
function StarBoxPanel:initUI(ui)
    ...
    self.node_reward:setPositionY(self.node_reward:getPositionY() + 8)
...
function StarBoxPanel:modifyWithDailyProgress(starNum)
    self.Txt_letter:setText(stringUtils.format(localizable.DailyMissionProgressTips,starNum))
    self.Img_star:setVisible(false)
    local fontWidth = self.Txt_letter:getContentSize()
    -- self.Txt_letter:setPositionX(fontWidth.width/2);
    self.Txt_letter:setPositionX(560);
    self.Txt_letter:setAnchorPoint(ccp(0.5, 0.5))
end
return StarBoxPanel;
