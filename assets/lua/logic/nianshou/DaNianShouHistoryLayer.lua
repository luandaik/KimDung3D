...
local DaNianShouHistoryLayer = class("DaNianShouHistoryLayer", BaseLayer)

function DaNianShouHistoryLayer:ctor(data)
    self.super.ctor(self)
    self:init("lua.uiconfig_mango_new.zadan.ZaDanHistory")
end


function DaNianShouHistoryLayer:initUI(ui)
    ...
    self.btn_otherRecord:setVisible(false)
    ...
    self.EggRecordList = DaNianShouManager:getHistoryList()
...
function DaNianShouHistoryLayer:registerEvents()
...
function DaNianShouHistoryLayer:removeEvents()
    self.super.removeEvents(self)
end


function DaNianShouHistoryLayer:removeUI()
    self.super.removeUI(self)
end

function DaNianShouHistoryLayer:onShow()
    self.super.onShow(self)

    self:refreshUI()
end

function DaNianShouHistoryLayer:dispose()
    self.super.dispose(self)
end

function DaNianShouHistoryLayer:refreshUI()

    self:drawMyResultLayer()
end

function DaNianShouHistoryLayer.OnclikMyHistory(sender)
    local self = sender.logic

end

function DaNianShouHistoryLayer.OnclikOtherHistory(sender)
    local self = sender.logic

end


function DaNianShouHistoryLayer:drawDefault(index)
...
function DaNianShouHistoryLayer:drawMyResultLayer()
...
function DaNianShouHistoryLayer.numberOfCellsInTableView(table)
    local self = table.logic

    return #self.EggRecordList
end

function DaNianShouHistoryLayer.cellSizeForTable(table,idx)
    local self = table.logic
    local height , width =   137, 718
    local index = idx + 1

    local totalCount = #self.EggRecordList
    ...
    if index  == (totalCount + 1) then
        ...
        local rewardData = self.EggRecordList[index]
        local countOfReward = 1
        local maxRow = math.ceil(countOfReward/5)
        width  = 730
        height = 240 + (maxRow - 1) * 160
    end

    return height , width
end

function DaNianShouHistoryLayer.tableCellAtIndex(table, idx)
    local self = table.logic
...
function DaNianShouHistoryLayer:createCellNode(cell, index)
...
function DaNianShouHistoryLayer:drawResultNode(cell, index)
    ...
    node:setVisible(true)
    local maxRow = math.ceil(countOfReward/5)
    node:setPositionY((maxRow - 1) * 160)
    ...
    local timestamp = math.floor(rewardData.dropTime/1000)
    ...
    for i=1,countOfReward do
        self:ShowRoleIcon(node, i, rewardData.itmes)
    end
...
function DaNianShouHistoryLayer:ShowRoleIcon(node, itemIndex, rewardList)
    local row = math.ceil(itemIndex/5)
    self.itemIndex = itemIndex

    local Gapx = 70 + 10
    local Gapy = 90 - (row - 1) * 160
    
    local posX = Gapx + math.mod(itemIndex-1,5) * 140
    local posY = Gapy

    local list = {}
    local item = string.split(rewardList,",")
    list.resType = tonumber(item[1])
    list.resId = tonumber(item[2])
    list.number = tonumber(item[3])
    print("list",list)
    ...
    -- end
        ...
        if item.isSpecial == 1 then
            local tdjTagImg = TFImage:create()
            roleQualityImg:addChild(tdjTagImg)
            tdjTagImg:setTexture('ui_new/zadan/img_tdj.png')
            tdjTagImg:setPosition(ccp(-39,39))
        end
    end
end

function DaNianShouHistoryLayer:loadMore()
    local nowCount = #self.EggRecordList
