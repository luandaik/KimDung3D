...
local rankrewardLayer = class("rankrewardLayer", BaseLayer)

function rankrewardLayer:ctor(data)
    self.super.ctor(self, data)
    self.myRank = data
    self:init("lua.uiconfig_mango_new.youli.ShaluReward")
end

function rankrewardLayer:initUI(ui)
    ...
    self.myRank = DaNianShouManager:getMyRank()
...
function rankrewardLayer:onShow()
    self.super.onShow(self)
end

function rankrewardLayer:registerEvents()
    self.super.registerEvents(self)
    ADD_ALERT_CLOSE_LISTENER(self,self.btn_close)
    self.btn_close:setClickAreaLength(100)
end

function rankrewardLayer:refreshUI()
    ...
    local rankConfig = ChampionsAwardData:getRewardData(25,self.myRank)
...
function rankrewardLayer:initTableData()
    self.dataList = ChampionsAwardData:getAllRewardDataByType(25) or {}
end

function rankrewardLayer:initTableView()
    self:initTableData()
    ...
    self.tableView:addMEListener(TFTABLEVIEW_SIZEFORINDEX, handler(rankrewardLayer.cellSizeForTable,self))
    self.tableView:addMEListener(TFTABLEVIEW_SIZEATINDEX, handler(rankrewardLayer.tableCellAtIndex,self))
    self.tableView:addMEListener(TFTABLEVIEW_NUMOFCELLSINTABLEVIEW, handler(rankrewardLayer.numberOfCellsInTableView,self))
    self.tableView:addMEListener(TFTABLEVIEW_SCROLL, handler(rankrewardLayer.tableScroll,self))
    self.Panel_list:addChild(tableView)
    self.tableView:reloadData()
end

function rankrewardLayer:cellSizeForTable(table,idx)
    return 112,600
end

function rankrewardLayer:tableCellAtIndex(table, idx)
    idx = idx + 1
...
function rankrewardLayer:numberOfCellsInTableView(table)
    local num = #(self.dataList)
    if num < 0 then num = 0 end
    return num
end

function rankrewardLayer:removeEvents()
    self.super.removeEvents(self)
end

function rankrewardLayer:dispose()
    self.super.dispose(self)
end

function rankrewardLayer:tableScroll(table)
...
function rankrewardLayer:setCellInfo(cell,idx)
