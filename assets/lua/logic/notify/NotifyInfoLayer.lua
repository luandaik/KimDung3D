...
function NotifyInfoLayer:infoBtnClickHandle(btn, btnIndex)
    ...
    elseif btnIndex == 3 then
        if HeitaoSdk then
            ...
            local notice_url = "http://"..MainPlayer:getHeiTaoUrl().."/affiche?pfid="..platformid
...
function NotifyInfoLayer:SetFightNotifyCell(cell, cellIndex)
    ...
    powerLabel:setColor(ccc3(0x3d, 0x3d, 0x3d))
    cell:addChild(powerLabel)

    local textLabel = TFTextArea:create()
    textLabel:setTextAreaSize(CCSizeMake(430,120))
    ...
    replayBtn:setPosition(ccp(665,60))
...
function NotifyInfoLayer:reloadData()
    ...
    self.infoList:reloadData()
    print("NotifyInfoLayer:reloadData()==========ssssss")
    if self.numberOfCellsInTableView() == 0 then
        if self.emptyImg == nil then
            self.emptyImg = TFImage:create("ui_new/notify/empty.png")
            self.emptyImg:setZOrder(100)
...
function NotifyInfoLayer:ShowReward(cell1, notifyInfo)
    ...
    for i=1,itemCount do
        ...
        elseif itemInfo.type == EnumDropType.TIANXIANG_COIN or 
            ...
            itemInfo.type == EnumDropType.HUIXIN or 
            itemInfo.type == EnumDropType.ZHUXIN or 
            itemInfo.type == EnumDropType.WEIWANG or 
            itemInfo.type == EnumDropType.FengYunLing then            
            itemQualityImg:setTexture(GetResourceQualityBG(itemInfo.type))

            local str = ""
            if itemInfo.type == EnumDropType.TIANXIANG_COIN then
                str = "ui_new/climb/icon_txb_big.png"
            elseif itemInfo.type == EnumDropType.TIANJI_COIN then
                str = "ui_new/climb/icon_tjb_big.png"
            elseif itemInfo.type == EnumDropType.HUIXIN then 
                str = "ui_new/shixintai/icon_hxb.png"
            elseif itemInfo.type == EnumDropType.ZHUXIN then 
                str = "ui_new/shixintai/icon_zxb.png"
            elseif itemInfo.type == EnumDropType.WEIWANG then
                str = "ui_new/hero/icon_wwz.png"
            elseif itemInfo.type == EnumDropType.FengYunLing then
                str = "ui_new/fengyunlu/ui_fyl.png"
            end

            local resIconImg = str
            if resIconImg then
        ...
        elseif itemInfo.type == EnumDropType.RECHARGE_TICKET then
            itemQualityImg:setTexture(GetResourceQualityBG(itemInfo.type))
            local id = 1
            if itemInfo.itemid then
                if itemInfo.itemid > 0 and itemInfo.itemid <= 6 then
                    id = itemInfo.itemid
                elseif itemInfo.itemid <= 12 then
                    id = itemInfo.itemid - 6
                end
            end
            local str = "ui_new/pay/ticket_icon" .. id .. ".png"
