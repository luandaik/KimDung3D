...
function VipLayer:showVipInfo( index )
    ...
    -- self.richText:setPositionY(size3.height - size1.height*0.5)
    -- self.sv_vipinfo:setContentOffset(ccp(0,size2.height - size3.height))

    if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
        self.richText:setPositionY(size3.height - size1.height)
        self.sv_vipinfo:setContentOffset(ccp(0, 0))
    else
        self.richText:setPositionY(size3.height - size1.height)
        self.sv_vipinfo:setContentOffset(ccp(0,size2.height - size3.height))
    ...
    -------------->  优化 领取之后变成购买vip礼包
    self.btn_free.isbuy = false;
    if MainPlayer:getVipLevel() >= vip.id and PayManager:isHaveGetVipReward(vip.id) then
        local constantInfo = ConstantData:objectByID("VIP_BOX_"..vip.id)
        if constantInfo then
            local vip_box_id = constantInfo.value
            local shopInfo = MallManager:getShopByResId( vip_box_id )
            if shopInfo and MallManager:isSellBox(shopInfo) then
                self.btn_free:setVisible(true);
                self.btn_free.isbuy = true;
                self.btn_free:setTextureNormal("ui_new/pay/btn_buy.png")
                self.btn_free:setTexturePressed("ui_new/pay/btn_buy.png")

                local rewardList = TFArray:new();
                rewardList:push(BaseDataManager:getReward({itemId = vip_box_id,type = EnumDropType.GOODS,number = 1}))
                for i=1,3 do
    ...
    self.btn_free:setTextureNormal("ui_new/pay/tq_mflq.png")
    self.btn_free:setTexturePressed("ui_new/pay/tq_mflq.png")
...
function VipLayer.onVipShopClickHandle(sender)
   local self = sender.logic;
...
function VipLayer.onGetReardClickHandle(sender)
   local self = sender.logic;
    ...
    if sender.isbuy then
        local constantInfo = ConstantData:objectByID("VIP_BOX_"..sender.vipId)
        if constantInfo then
            local vip_box_id = constantInfo.value
            local shopInfo = MallManager:getShopByResId( vip_box_id )
            -- MallManager:checkShopOneKey( vip_box_id )
            if shopInfo then
