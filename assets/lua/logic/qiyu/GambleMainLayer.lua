...
function GambleMainLayer:removeEvents()
    ...
    if GambleManager.batchBetAuto then
        local length = 0
        if RewardManager.data and RewardManager.data.items then
            length = #RewardManager.data.items - 1
        end
        if RewardManager:setStopShow(false)  then
            local layer = AlertManager:getLayerByName("lua.logic.common.RewardListMessage")
            if layer then
                layer:setTextSHow(stringUtils.format(localizable.GambleMainLayer_OneKeyTxt,length,GambleManager.batchBetAutoCount ))
            end
        end
        GambleManager.batchBetAuto = false
    end
...
function GambleMainLayer.gambleBtnClickHandle(sender)
    ...
    if GambleManager:getStateByIndex(index ) then
        ...
        if gambleInfo then
            if index >= 4 then
                if not VipRuleManager:showUsedGoldTimeLayer( MainPlayer:getVipRuleTimesByCode( 8101+index-4 ) ,8101+index-4, gambleInfo.title) then
                    return
                end
            end
            local token = gambleInfo:getToken()
            ...
            if token then
                if token.type == EnumDropType.GOODS then
                    ...
                    if resValue >= token.value then
                        ...
                        if GambleManager.gamble_cost_token_tip[index] == false then
                            local layer = CommonManager:showOperateCostSureTipLayer(
                                function(data, widget)
                                    GambleManager:requestBetByType(index)
                                    GambleManager.gamble_cost_token_tip[index] = widget:getSelectedState() or false;
                                end,
                                function(data, widget)
                                    AlertManager:close()
                                    GambleManager.gamble_cost_token_tip[index] = widget:getSelectedState() or false;
                                end,
                                {
                                    ...
                                    msg = stringUtils.format(localizable.GambleMainLayer_text2,gambleInfo.title,token.value,tokenInfo.name ),
                            ...
                            if index >= 4 then
                                ...
                                if vipInfo then
                                    ...
                                    layer:showSureTip(stringUtils.format(localizable.GambleMainLayer_text4 ,gambleInfo.title, maxBuyTime))
                                end
                            end
                            return
                        end
                ...
                if consumes.type == EnumDropType.SYCEE then
                    if GambleManager.gamble_cost_tip[index] == false then
                        ...
                        if index >= 4 then
                            ...
                            if vipInfo then
                                local maxBuyTime = vipInfo.benefit_value - MainPlayer:getVipRuleTimesByCode( 8101+index-4 )
                                layer:showFreshTip(stringUtils.format(localizable.GambleMainLayer_text4,gambleInfo.title , maxBuyTime))
                            end
                        end
                        return
...
function GambleMainLayer.zhenxuanBtnClickHandle(sender)
    ...
    if token then
        if token.type == EnumDropType.GOODS then
            ...
            if resValue >= token.value then
                local tokenInfo = ItemData:objectByID(token.id)
                if GambleManager.zhenxuan_cost_token_tip == false then
                    CommonManager:showOperateCostSureTipLayer(
                        function(data, widget)
                            GambleManager:requestPick()
                            GambleManager.zhenxuan_cost_token_tip = widget:getSelectedState() or false;
                        end,
                        function(data, widget)
                            AlertManager:close()
                            GambleManager.zhenxuan_cost_token_tip = widget:getSelectedState() or false;
                        end,
                        {
                            ...
                            title=localizable.TreasureMain_tips1,
                            --msg = gambleInfo.title.."需要花费"..consumes.value..GetResourceName(consumes.type).."，是否确认",
                            msg = stringUtils.format(localizable.GambleMainLayer_text3,token.value,tokenInfo.name ),
                            showtype = AlertManager.BLOCK_AND_GRAY,
                            costValue = token.value,
...
function GambleMainLayer.yjdsBtnClickHandle(sender)
    ...
    AlertManager:addLayerByFile("lua.logic.qiyu.GambleChoose",AlertManager.BLOCK_AND_GRAY,AlertManager.TWEEN_0);
    AlertManager:show();
...
function GambleMainLayer:addGetEffect(cell,node)
    ...
    if GambleManager.batchBetAuto then
        effect:addMEListener(TFARMATURE_COMPLETE,function()
                self:setBatchBetAutoFalse()
            end)
    end
end

function GambleMainLayer:setBatchBetAutoFalse()
    if not GambleManager.batchBetAuto then
        return
    end
    print("GambleMainLayer:setBatchBetAutoFalse()")
    GambleManager.batchBetAuto = false
    self:refreshBagView()
    local length = 0
    if RewardManager.data and RewardManager.data.items then
...
function GambleMainLayer.numberOfCellsInTableView(table)
    local num = math.ceil(GambleManager:getBagNum()/column)
    num = math.min(num ,5)
    return num
end
