...
local JiayuanInviteAcceptLayer = class("JiayuanInviteAcceptLayer", BaseLayer)

function JiayuanInviteAcceptLayer:ctor()
    self.super.ctor(self)
    self:init("lua.uiconfig_mango_new.qiyu.CodeLayerAccept")
end

function JiayuanInviteAcceptLayer:initUI(ui)
...
function JiayuanInviteAcceptLayer:registerEvents(ui)
...
function JiayuanInviteAcceptLayer:removeEvents()
    self.super.removeEvents(self)
end


function JiayuanInviteAcceptLayer:onShow()
    self.super.onShow(self)
    self:refreshUI()
end


function JiayuanInviteAcceptLayer:refreshUI()
...
function JiayuanInviteAcceptLayer.onclickVerification(sender)
    ...
    if self.Invited ~= true then
        ...
        if string.len(code) == 0 then
            ...
            if string.find(code, "^-?[1-9]%d*$") then
                QiyuManager:VerifyInviteCode( tonumber(code) )
            else
...
function JiayuanInviteAcceptLayer.onclickReward(sender)
    local self = sender.logic
...
function JiayuanInviteAcceptLayer:drawRewardList()
...
function JiayuanInviteAcceptLayer.numberOfCellsInTableView(table)
...
function JiayuanInviteAcceptLayer.cellSizeForTable(table,idx)
    return 200, 120
end

function JiayuanInviteAcceptLayer.tableCellAtIndex(table, idx)
    local self = table.logic
...
function JiayuanInviteAcceptLayer:drawInviteNode(node)
...
return JiayuanInviteAcceptLayer