...
local JiayuanInviteSendLayer = class("JiayuanInviteSendLayer", BaseLayer)
...
function JiayuanInviteSendLayer:removeEvents()
    self.super.removeEvents(self)
end


function JiayuanInviteSendLayer:onShow()
    self.super.onShow(self)
    self:refreshUI()
end


function JiayuanInviteSendLayer:refreshUI()
...
function JiayuanInviteSendLayer:creatRichText()
    ...
    local strFormat1 = [[<p style="text-align:left margin:5px"><font face = "simhei" color="#3d3d3d" fontSize="26">%s</font><font face = "simhei" color="#008030" fontSize="26">%s]]
    local strFormat2 =   [[</font><font face = "simhei" color="#3d3d3d" fontSize="26">%s</font><font face = "simhei" color="#008030" fontSize="26">%s]]
    local strFormat3 =    [[</font><font face = "simhei" color="#3d3d3d" fontSize="26">%s</font><font face = "simhei" color="#008030" fontSize="26">%s</font><font face = "simhei" color="#3d3d3d" fontSize="26">%s</font></p>]]
    local strFormat = strFormat1 .. strFormat2 .. strFormat3

    local notifyStr = ""
    notifyStr = string.format(strFormat, desc1, serverName, desc2, myName, desc3, InviteCode, desc4)
...
function JiayuanInviteSendLayer:copyToPasteBord()
...
function JiayuanInviteSendLayer.CopyContent(sender)
...
function JiayuanInviteSendLayer.sendToFriend(sender)
    local self = sender.logic

    self:copyToPasteBord()
    --toastMessage("大侠已复制成功，快去您的微信分享吧！")
    --toastMessage(localizable.InFriendSendLayer_share)
    QiyuManager:goShare2()
end

return JiayuanInviteSendLayer