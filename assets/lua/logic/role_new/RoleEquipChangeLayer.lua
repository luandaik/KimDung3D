...
function RoleEquipChangeLayer:ctor(data)
    self.super.ctor(self)
    self.roleId = {}
    self.fightType = data or EnumFightStrategyType.StrategyType_PVE
...
function RoleEquipChangeLayer:initUI(ui)
    ...
    for i=1,2 do
        ...
        for j=1,6 do
            ...
            local img_cut = TFDirector:getChildByPath(panel, 'img_cut')
            if verLock and verLock.open == 0 and j == 6 then
                self.EquipList[i][j].root_Node:setVisible(false)
                img_cut:setVisible(false)
            end
        end
...
function RoleEquipChangeLayer:refreshRoleInfo(index)
    ...
    if self.roleId[index] then
        ...
        if cardRole then
            AssistFightManager:freshRoleInStrategyPower(self.fightType,cardRole.gmId)
            ...
            self.EquipList[index].txt_power:setText(cardRole:getPowerByFightType(self.fightType))
        end
    end
...
function RoleEquipChangeLayer:getRoleList()
    ...
    for roleId in self.roleList:iterator() do
        if roleId ~= self.roleId[1] and roleId ~= self.roleId[2] then
...
function RoleEquipChangeLayer:setRoleList(roleList)
    self.roleList = clone(roleList)
end
