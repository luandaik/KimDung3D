...
function RoleBook_OnEquip:initUI(ui)
    ...
    local txt_bookdesc      = TFDirector:getChildByPath(ui, 'txt_bookdesc')
    self.txt_warning        = TFDirector:getChildByPath(ui, 'txt_warning')
    self.img_bookQuality:setTouchEnabled(false)
    self.txt_bookdesc = Public:createRichtextHB(txt_bookdesc)
    -- 合成相关
...
function RoleBook_OnEquip:registerEvents(ui)
    ...
    self.MartialSynthesisBrushErrorEventCallBak = function(event)
        self:refreshUI()
        self.layer_hecheng:redraw()
    end
    TFDirector:addMEGlobalListener(MartialManager.MSG_MartialBrushError ,self.MartialSynthesisBrushErrorEventCallBak )
...
function RoleBook_OnEquip:removeEvents()
    ...
    TFDirector:removeMEGlobalListener(MartialManager.MSG_MartialBrushError ,self.MartialSynthesisBrushErrorEventCallBak )
    self.MartialSynthesisBrushErrorEventCallBak = nil
...
function RoleBook_OnEquip:drawLeftArea()
    ...
    self.txt_bookdesc:setText(stringUtils.richText(bookInfo.goodsTemplate.details))
...
function RoleBook_OnEquip:addXieDeEffect(effectName)
    ...
    if self.bookStatus == 1 and self.windowStatus == 1 then
        if self.effect == nil then
            ...
            effect:setPosition(ccp(270-8, 120-10))
            self.effect = effect
        end
    ...
    if self.bookStatus == 1 and self.windowStatus == 1 then
        if self.btn_qxkl.effect then
            self.btn_qxkl.effect:playByIndex(0, -1, -1, 1)
            return
        end
        if self.btn_qxkl.effect == nil then
            print("addd effect")
            TFResourceHelper:instance():addArmatureFromJsonFile("effect/"..effectName..".xml")
            local effect = TFArmature:create(effectName.."_anim")
            effect:setAnimationFps(GameConfig.ANIM_FPS)
            ...
            self.btn_qxkl:addChild(effect, 100)
            --self:addChild(effect , 100)
            --local size = self.btn_qxkl:getSize()
            effect:setPosition(ccp(0, 0))
            self.btn_qxkl.effect = effect
        end

    else

        if self.btn_qxkl.effect then
            self.btn_qxkl.effect:removeFromParent()
            self.btn_qxkl.effect = nil
        end
    end
end
