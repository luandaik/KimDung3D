...
function GetTenRoleResultLayer:initUI(ui)
	...
	self.roleNode = TFDirector:getChildByPath(self.ui, "img_quality_bg")
	self.roleNode:setVisible(false)
	self.img_ewai = TFDirector:getChildByPath(self.ui, "img_ewai")
	self.img_ewai:setVisible(false)
	self.ui = ui
end
...
function GetTenRoleResultLayer:registerEvents(ui)
	...
	local yuanBaoCost = GetRolePriceData:getNowPrice( 3 , todayTimes + 1 )
...
function GetTenRoleResultLayer:OnIconShowEnd()
	if self.roleIndex > #GetCardManager.getCardTypeList then
		...
		if GetCardManager.getCardTypeList[self.roleIndex] == nil then
			print("cannot find the index = ",self.roleIndex)
			return
		end
		local roleTypeId = GetCardManager.getCardTypeList[self.roleIndex].resId
		if roleTypeId == nil then
			print("GetCardManager.getCardTypeList = ",GetCardManager.getCardTypeList)
			print("self.roleIndex = ",self.roleIndex)
			return
		end
...
function GetTenRoleResultLayer:ShowRoleIcon(roleIndex)
	...
	if newCardRoleData ~= nil then
		local target = nil
		local scale = 1
		if roleIndex == 11 then
			self.roleNode:setVisible(true)
			self.img_ewai:setVisible(true)
			self.roleNode:setTexture(GetColorIconByQuality(newCardRoleData.quality))
			local img_icon = TFDirector:getChildByPath(self.roleNode, "img_icon")
			...
			local txt_num = TFDirector:getChildByPath(self.roleNode, "txt_num")
			if item.resType == EnumDropType.GOODS then
				...
				if newCardRoleData.type == EnumGameItemType.Soul and newCardRoleData.kind ~= 3 then
					Public:addPieceImg(img_icon,newCardRoleData,true,1)
