...
function ChongzhuCailiao:initDateByFilter( gmId, equip_list, clickCallBack, recastItem)

    self.equipList = equip_list or TFArray:new()
    self.bagItem = nil
    self.recastItem = recastItem

    -- local item = recastItem:objectAt(2)
    -- if item then
    --     local id = item.itemid
    --     self.bagItem = BagManager:getItemById(id)
    -- end
