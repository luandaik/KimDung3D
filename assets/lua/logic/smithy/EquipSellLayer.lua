...
function EquipSellLayer.tableCellAtIndex(table, idx)
    ...
    for i=1,5 do
    	...
    	if (index) <= self.equipList:length() then
			...
			AutoTestManager:addAutoTestName(cell.equip_panel[i], "SmithyEquipIcon_"..index)
    		cell.equip_panel[i]:setEquipGmId(equip.gmId)
    		cell.equip_panel[i]:setDuigouVisiable(self.selectIconTable[equip.gmId])
    	else
			AutoTestManager:addAutoTestName(cell.equip_panel[i], "")
...
function EquipSellLayer:refreshEquipList()
	...
	if self.tableView then
		...
		AutoTestManager:addAutoTestName(self.tableView, "equipListLen_"..self.equipList:length())
	end
end
