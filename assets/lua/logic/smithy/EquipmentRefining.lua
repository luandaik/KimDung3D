...
function EquipmentRefining:initUI(ui)
    ...
    self.btn_jinglianxiaohao = TFDirector:getChildByPath(ui, 'btn_jinglianxiaohao')
    self.btn_jinglianxiaohao:setVisible(false)
    self.btn_tupoxiaohao = TFDirector:getChildByPath(ui, 'btn_tupoxiaohao')
    self.btn_tupoxiaohao:setVisible(false)
    self.btn_tupoxiaohao2 = TFDirector:getChildByPath(ui, 'btn_tupoxiaohao2')
    self.btn_tupoxiaohao2:setVisible(false)

    self.node_tupo_consumeList = {}
end
...
function EquipmentRefining.RefiningAutoClickHandle(sender)
    ...
    if MainPlayer:getVipLevel() < self.lockVip.value then
        local msg =  stringUtils.format(localizable.vip_yiJianJingLian_not_enough,self.lockVip.value);
        CommonManager:showOperateSureLayer(
                function()
    ...
    if stoneNumber < 1 then
        ...
        -- 
        local consumeGoods = EquipmentManager:getRefinStone() or {}
        if ResGetWayManager:openResGetLayer(consumeGoods.id,EnumDropType.GOODS) == true then
            return
        end
	toastMessage(localizable.smithy_EquipmentRefining_jlsbuzu)
...
function EquipmentRefining:refreshConsumeGoodsRemaining()
    self.refinStone = 0
    if self.node_consume == nil then
        self.node_consume = require('lua.logic.role_new.RoleStarUpPreviewNumCell'):new()
        self.node_consume:setPosition(ccp(427,128))
        self.scroll_right:addChild(self.node_consume)
    end
    local consumeGoods = ConstantData:objectByID("Equip.Refining.Consume.Goods")
    ...
    self.node_consume:setData(consumeGoods.res_id,holdGoods_num,consumeGoods.value,consumeGoods.res_type)
...
function EquipmentRefining:refrshTupoArea()
    ...
    local consumeList = {}
    self.tupoCostList = {}
    if tupInfo.consume == nil or tupInfo.consume == "" then
        for k,v in pairs(self.node_tupo_consumeList) do
            v:setVisible(false)
        end
    else
        local tbl = string.split(tupInfo.consume,'|')
        local index = 0
        for i,v in ipairs(tbl) do
            index = index + 1
            --local consume   = string.split(tupInfo.consume,'_')
            local consume   = string.split(v,'_')
            print("consume = ", consume)
            local costType  = tonumber(consume[1])
            local itemId    = tonumber(consume[2])
            local number    = tonumber(consume[3])
            
            local numberInBag = BagManager:getItemNumById(itemId)
            self.tupoCostList[i] = {itemId = itemId,type = costType,number = number,numberInBag = numberInBag}

            if self.node_tupo_consumeList[i] == nil then
                self.node_tupo_consumeList[i] = require('lua.logic.role_new.RoleStarUpPreviewNumCell'):new()
                self.scroll_right:addChild(self.node_tupo_consumeList[i])
            end
            self.node_tupo_consumeList[i]:setData(itemId,numberInBag,number,costType)
        end
        for i=1,#self.node_tupo_consumeList do
            if i <= index then
                self.node_tupo_consumeList[i]:setVisible(true)
                local posx = 40 - ((index - 1)/2) * 88 + (i - 1) * 88
                self.node_tupo_consumeList[i]:setPosition(posx,128)
            else
                self.node_tupo_consumeList[i]:setVisible(false)
            end
        end
    end
...
function EquipmentRefining.TupoOnclick( sender )
    ...
    local itemList = {}
    for i=1,#self.tupoCostList do
        local stoneNumber = self.tupoCostList[i].numberInBag
        local costNumber  = self.tupoCostList[i].number
        if costNumber > stoneNumber then
            table.insert(itemList,{self.tupoCostList[i].itemId, self.tupoCostList[i].type})
            -- self:showMallLayer()
            --toastMessage("物品不足")
            --toastMessage(localizable.smithy_EquipmentRefining_pro)
            --MallManager:checkShopOneKey(self.tupoCostList[i].itemId)
            --return
        end
    end
    if #itemList > 0 then
