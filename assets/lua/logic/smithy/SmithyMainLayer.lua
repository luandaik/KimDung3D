...
function SmithyMainLayer:initUI(ui)
    ...
    -- 一键强化
    self.btn_yjqh = TFDirector:getChildByPath(ui, 'btn_yjqh')
    self.btn_yjqh.logic = self
	...
	for i = 1,10 do
		if fightMatix[i] and fightMatix[i] ~= 0 then
			...
			if role then
		        ...
		        CommonManager:setRedPoint(self.icon_tab[tmpIdx], redpoint,"IsCanSmithy",ccp(13,-3))
			end
			if tmpIdx == 5 then
				break
			end
...
function SmithyMainLayer.tableCellAtIndex(table, idx)
    ...
    for i=1,5 do
        CommonManager:setRedPoint(cell.equip_panel[i].img_icon, false,"IsCanSmithy",ccp(15,15))
    	if (idx * 5 + i) <= self.equipList:length() then
	    	...
	    	if equip.equip ~= 0 then
	    		...
	    		if role and role.pos ~= nil and role.pos > 0 then
	    			local curEquipLevel = equip.level + 1
	    			if (equip.level == 150) then
	    				--如果是最大等级就不加1
	    				curEquipLevel = 150
	    			end
	    			local coin = IntensifyData:getConsumeByIntensifyLevel(curEquipLevel, equip.quality)
		            local bTemp = MainPlayer:isEnoughCoin(coin, false)

		            if bTemp and equip.level < MainPlayer:getMaxIntensifyLevel() then
		            	CommonManager:setRedPoint(cell.equip_panel[i].img_icon, true,"IsCanSmithy",ccp(15,15))
		            else
		            	local bxiangqian = KaojiaoManager:checkEquipXiangqianByEquipId(equip.gmId,false)
		            	CommonManager:setRedPoint(cell.equip_panel[i].img_icon, bxiangqian,"IsCanSmithy",ccp(15,15))
		            end
	    		end
	    	else
	    		CommonManager:setRedPoint(cell.equip_panel[i].img_icon, false,"IsCanSmithy",ccp(15,15))
...
function SmithyMainLayer.onBtnStrengthen(sender)
	local self = sender.logic
	local selectedRole = StrategyManager:getFightRoleBySequence(self.selectedTab)
	if selectedRole == nil then
		return
	...
	CommonManager:showOperateSureLayer(
        function()
            ...
            for v in selectedRole:getEquipment():iterator() do
            	if v.level < maxIntensifyLevel then
					local coin = IntensifyData:getConsumeByIntensifyLevel(v.level + 1,v.quality)
					if coin ~= nil then
						EquipmentManager:EquipmentIntensifyToTop(v.gmId)
					end
				end
			end
        end,
        nil,
        {
            ...
            msg = localizable.smithyIntensify_onekey,
...
function SmithyMainLayer:registerEvents()
    ...
    self.btn_yjqh:addMEListener(TFWIDGET_CLICK, audioClickfun(self.onBtnStrengthen))
...
function SmithyMainLayer:removeEvents()
	...
	self.equipmentIntensifyCallBack = nil
