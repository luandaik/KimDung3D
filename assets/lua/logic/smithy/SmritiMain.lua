...
function SmritiMain:initUI( ui )
    ...
    for i=1,2 do
    	...
    	panel_details.base_attr.txt_base = TFDirector:getChildByPath(detailsNode,'txt_baseattr_index')
    	panel_details.base_attr.txt_base_val = TFDirector:getChildByPath(detailsNode,'txt_baseattr_num')
    	--附加属性
        panel_details.attr_extra ={}
    	for i=1,4 do
    		local attr_extra = {}
    		attr_extra.txt_extra = TFDirector:getChildByPath(detailsNode,'txt_extraattr_'..i)
    		attr_extra.txt_extra_val = TFDirector:getChildByPath(detailsNode,'txt_extraattr_num_'..i)
...
function SmritiMain:refreshUI()
	for index,gmId in pairs(self.gmIds) do
		if gmId ~= 0 then			
            ...
            if notEmptyIndex == 1 then
                offsetOrderPanelY = attr_extra[1].txt_extra:getPositionY()
            else
                offsetOrderPanelY = attr_extra[notEmptyIndex-1].txt_extra:getPositionY() - 30
            end
            Public:showAttributeMove( self.panelMove[index], equip )   
            Public:showOrderDetailInfo(self.panelMove[index],  equip, offsetOrderPanelY)  
	        --宝石
    ...
    if self.gmIds[1] ~=0 and self.gmIds[2] ~= 0 then    
        ...
        self.items ={}
        local temquality = EquipmentManager:getEquipByGmid(self.gmIds[2])
        local equip = EquipmentManager:getEquipByGmid(self.gmIds[1])
        if temquality.quality > equip.quality then
            equip = temquality
        end
...
function SmritiMain:registerEvents()
    ...
    self.smritiCallBack =  function (event)
        ...
        for i = 1, #self.gmIds do
            local equip = EquipmentManager:getEquipByGmid(self.gmIds[i])
            if equip then
                equip:UpdateAttr()
                if equip.equip then
