...
--
local BibleSelect = class("BibleSelect",BaseLayer)

function BibleSelect:ctor(data)
	self.super.ctor(self, data)
	self:init("lua.uiconfig_mango_new.smithy.SmritiSelect")
    self.selectGmId = 0 
end

function BibleSelect:initUI( ui )
...
function BibleSelect:initTableview()
    ...
    tableView:addMEListener(TFTABLEVIEW_SIZEFORINDEX, BibleSelect.cellSizeForTable)
    tableView:addMEListener(TFTABLEVIEW_SIZEATINDEX, BibleSelect.tableCellAtIndex)
    tableView:addMEListener(TFTABLEVIEW_NUMOFCELLSINTABLEVIEW, BibleSelect.numberOfCellsInTableView)
    tableView:addMEListener(TFTABLEVIEW_SCROLL, BibleSelect.tableScroll)
	self.panel_list:addChild(tableView)

    self.tableView:reloadData()
end

function BibleSelect.cellSizeForTable(table,idx)
    return 216,725
end
function BibleSelect.tableCellAtIndex(table, idx)
...
function BibleSelect.numberOfCellsInTableView(table)
	local self = table.logic
...
function BibleSelect.tableScroll(table)
    local self = table.logic
...
function BibleSelect.iconClickHandle(sender)
...
function BibleSelect:loadItemNode(item_node,equipItem)
...
function BibleSelect:setParent(parent)
	self.parent = parent
end

function BibleSelect:setSelectId(selectGmId) 
	--self.parent:setSelectId(selectGmId)
    self.selectGmId = selectGmId
	self.onBtnClose(self.btn_close)
end

function BibleSelect:setInstanceId(instanceId)
...
function BibleSelect.onBtnClose(sender)
    local self = sender.logic
...
function BibleSelect:removeUI()
   	self.super.removeUI(self)  
end

function BibleSelect:onShow()
    self.super.onShow(self)
    self.ui:runAnimation("Action0",1)
end

function BibleSelect:registerEvents()
...
function BibleSelect:removeEvents()
	self.super.removeEvents(self)
end


function BibleSelect:dispose()
