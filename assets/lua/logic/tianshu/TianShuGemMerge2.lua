-- client side TianShuGemMerge2.lua
--[[
...
local TianShuGemMerge2 = class("TianShuGemMerge2", BaseLayer)

local columnNumber = 4

function TianShuGemMerge2:ctor(data)
    self.super.ctor(self,data)
    self:init("lua.uiconfig_mango_new.tianshu.TianShuGemMerge2")
end

function TianShuGemMerge2:initUI(ui)
	self.super.initUI(self,ui)

    --左侧详情
    self.scroll_left            = TFDirector:getChildByPath(ui, 'scroll_left')
    self.info_panel             = require('lua.logic.tianshu.TianShuGemMerge'):new()
...
function TianShuGemMerge2:onShow()
    self.super.onShow(self)
    self:refreshUI()
    self.info_panel:onShow()
end

function TianShuGemMerge2:refreshUI()
...
function TianShuGemMerge2:dispose()
    self.info_panel:dispose()
    self:disposeAllPanels()
    self.super.dispose(self)
end

function TianShuGemMerge2:removeUI()
	self.super.removeUI(self)
end

function TianShuGemMerge2:refreshButtonState()
...
function TianShuGemMerge2:refreshBtnCost()
    if self.selectId == nil or self.selectId == 0 then 
...
function TianShuGemMerge2:refreshTargetGem()
    if self.selectId == nil or self.selectId == 0 then 
...
function TianShuGemMerge2:disposeAllPanels()
...
function TianShuGemMerge2:updateTableSource()
...
function TianShuGemMerge2:unselectedAllCellPanels()
    if self.allPanels == nil then
        return
    end
...
function TianShuGemMerge2:updateSelectCell()
    if self.allPanels == nil then
        return
    end
...
function TianShuGemMerge2:selectDefault()
    if self.allPanels == nil then
        self.selectId = nil
        return
    end
...
function TianShuGemMerge2:select(id)
    --选中的Cell如果重复选中则不处理
...
function TianShuGemMerge2.cellSizeForTable(table,idx)    
    return 200,540
end

function TianShuGemMerge2.tableCellAtIndex(table, idx)
    ...
    for i=1,columnNumber do
        ...
        --print("TianShuGemMerge2.tableCellAtIndex : ",self.gemlist:length(),atIndex)
...
function TianShuGemMerge2:showOneKeyEffect( level )
...
function TianShuGemMerge2.numberOfCellsInTableView(table)
    local self = table.logic
    if self.gemlist and self.gemlist:length() > 0 then
...
function TianShuGemMerge2:initTableview()
    ...
    tableView:addMEListener(TFTABLEVIEW_SIZEFORINDEX, TianShuGemMerge2.cellSizeForTable)
    tableView:addMEListener(TFTABLEVIEW_SIZEATINDEX, TianShuGemMerge2.tableCellAtIndex)
    tableView:addMEListener(TFTABLEVIEW_NUMOFCELLSINTABLEVIEW, TianShuGemMerge2.numberOfCellsInTableView)

    --self:updateTableSource()
    self.panel_list:addChild(tableView)
end

function TianShuGemMerge2:gemBuild()
    if self.selectId == nil or self.selectId == 0 then
...
function TianShuGemMerge2.gemBuildAutoBtnClickHandle(btn)
...
function TianShuGemMerge2.oneKeyBuild(btn )
    local self = btn.logic
    local has = false
    for v in self.gemlist:iterator() do
        local itemInfo = ItemData:objectByID(v.id)
        if v.num >= 4 and itemInfo and itemInfo.level < 9 then
            has = true
        end
    end
    if has == false then
...
function TianShuGemMerge2.gemBuildBtnClickHandle(btn)
	local self = btn.logic
...
function TianShuGemMerge2:iconBtnClick(cell)
    if self.isauto then
...
function TianShuGemMerge2:buildGemAction()
    self.actionpaly = true
    self.percent = 0
...
function TianShuGemMerge2:choiceGemAction(cell)
...
function TianShuGemMerge2:choiceGemActionPlay(pic,path,pos,index)
...
function TianShuGemMerge2:playEffect(x,y)
...
function TianShuGemMerge2:_choiceGemAction(list)
    local id = list:pop()
...
function TianShuGemMerge2:_choiceGemActionPlay(pic,path,pos,list)
    play_hechengbaoshiqianru()
    pic:setPosition(pos)
        ...
        target = pic,
            {
                ...
                onComplete = function ()
                    ...
                    if list:length() > 0 then
                        ...
                        BagManager:autoMergeGemRequest(9)
                        self:playMergeEffect()
                    end
                end,
            },
...
function TianShuGemMerge2:_playEffect(panel)
    TFResourceHelper:instance():addArmatureFromJsonFile("effect/ui/gem_click.xml")
    local effect = TFArmature:create("gem_click_anim")
    if effect == nil then
        return
...
function TianShuGemMerge2:playMergeEffect()
...
function TianShuGemMerge2:registerEvents()
...
function TianShuGemMerge2:removeEvents()
...
function TianShuGemMerge2:stopAutoBuild()
    self.isauto = false
    self.itemNeedChang = true
end

--显示升星成功对话框
function TianShuGemMerge2:showSuccessDialog()
...
function TianShuGemMerge2:isTouchInTableView(position)
