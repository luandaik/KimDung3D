...
function TianshuStarupLayer:refreshUI()
    ...
    for i = 1, EnumAttributeType.Max - 1 do       
        if self.newarr[i] and self.newarr[i] ~= 0 and count < SkyBookManager.kMaxAttributeSize then
            ...
            local str1 = string.format("(%d)", self.oldarr[i] or 0)
            local str2 = string.format("(%d)", self.newarr[i] or 0)
            self.txt_att_old[count]:setText(self.oldFactor .. str1)
            self.txt_att_new[count]:setText(newFac .. str2)
        end
    end
    ...
    for i=1,self.old_starlevel + 1 do
        ...
        if (self.img_star_h[starIdx]) then
            self.img_star_h[starIdx]:setTexture(starTextrue)
...
function TianshuStarupLayer:drawStarAction()
    ...
    for i=1,SkyBookManager.kMaxStarLevel do
        ...
        if i == self.item:getTupoLevel() then
            local starIdx = i>5 and i-5 or i
            ...
            self.img_star_h[starIdx]:setTexture(starTextrue)
            self.img_star_h[starIdx]:setVisible(true)
            self:playStarActivity(self.img_star_h[starIdx])
            ...
            effect:setPosition(ccp(pos.x+24, pos.y+21))
            effect:setZOrder(1)
...
function TianshuStarupLayer:playStarActivity(node)
    if (node == nil) then
        return
    end
    node:setOpacity(0)
