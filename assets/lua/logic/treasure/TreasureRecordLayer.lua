-- client side TreasureRecordLayer.lua
--[[
 * @Description: 寻宝历史
 ]]
...
local TreasureRecordLayer = class("TreasureRecordLayer", BaseLayer)

function TreasureRecordLayer:ctor(data)
    self.super.ctor(self)
    self:init("lua.uiconfig_mango_new.treasure.TreasureHistory")
end


function TreasureRecordLayer:initUI(ui)
...
function TreasureRecordLayer:registerEvents()
...
function TreasureRecordLayer:removeEvents()
...
function TreasureRecordLayer:removeUI()
    self.super.removeUI(self)
end

function TreasureRecordLayer:onShow()
    self.super.onShow(self)

    -- self:refreshUI()
end

function TreasureRecordLayer:dispose()
    self.super.dispose(self)
end

function TreasureRecordLayer:refreshUI()

    self:drawMyResultLayer()
end

function TreasureRecordLayer.OnclikMyHistory(sender)
    local self = sender.logic

end

function TreasureRecordLayer.OnclikOtherHistory(sender)
    local self = sender.logic

end


function TreasureRecordLayer:drawDefault(index)
...
function TreasureRecordLayer:onClickDay(index)
...
function TreasureRecordLayer.onClickTpye(sender)
    local self  = sender.logic
...
function TreasureRecordLayer:drawMyResultLayer()
...
function TreasureRecordLayer.numberOfCellsInTableView(table)
    local self = table.logic

    return #self.recordList + 1
end

function TreasureRecordLayer.cellSizeForTable(table,idx)
    local self = table.logic
...
function TreasureRecordLayer.tableCellAtIndex(table, idx)
    local self = table.logic
...
function TreasureRecordLayer:createCellNode(cell, index)
    local node = nil
    if index == 3 then
        node = CCNode:create()
    else
...
function TreasureRecordLayer:drawResultNode(cell, index)
    local node = nil
...
function TreasureRecordLayer:ShowRoleIcon(node, itemIndex, rewardList)
    ...
    if newCardRoleData ~= nil then
        ...
        if item.resType == EnumDropType.GOODS then
            ...
            if newCardRoleData.type == EnumGameItemType.Soul and newCardRoleData.kind ~= 3 then
                Public:addPieceImg(roleIcon,newCardRoleData,true,1.0)
            elseif newCardRoleData.type == EnumGameItemType.Piece then
                Public:addPieceImg(roleIcon,newCardRoleData,true,1.0)
            else
                Public:addPieceImg(roleIcon,newCardRoleData,false,1.0)
            end
...
function TreasureRecordLayer:addTag(node,num)
...
function TreasureRecordLayer:loadMore()
