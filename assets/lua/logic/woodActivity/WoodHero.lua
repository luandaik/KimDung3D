...
local WoodHero = class("WoodHero", BaseLayer)

function WoodHero:ctor(data)
	--print(data)
    self.roleId = data[1]
...
function WoodHero:registerEvents(ui)
	...
	if self.roleIndex then
		...
		audioClickfun(function(sender)
            ...
            AlertManager:close()
			if parent ~= nil and parent.ShowRoleIcon then
...
function WoodHero:removeEvents()
...
function WoodHero:setParent(parent)
	self.parent = parent
end

function WoodHero:PlayStartEffect()
	...
	function() 
		...
		TFDirector:removeTimer(self.timerID)
		self.timerID = nil
		if self.PlayShowRoleEffect then
...
function WoodHero:PlayShowRoleEffect()
...
function WoodHero:DrawAttrPolygon(attribute_level)
...
function WoodHero:ShowRoleDetail(newCardRoleData)
...
function WoodHero:OnRoleShowEnd(newCardRoleData)
...
function WoodHero:setReturnFun( fun )
	-- self.returnFun = fun
