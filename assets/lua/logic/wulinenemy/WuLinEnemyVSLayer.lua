-- client side WuLinEnemyVSLayer.lua
--[[
...
local WuLinEnemyVSLayer = class("WuLinEnemyVSLayer", BaseLayer)
-- local CardRole = require('lua.gamedata.base.CardRole')
CREATE_SCENE_FUN(WuLinEnemyVSLayer)
CREATE_PANEL_FUN(WuLinEnemyVSLayer)

WuLinEnemyVSLayer.LIST_ITEM_WIDTH = 200 

function WuLinEnemyVSLayer:ctor(data)
    self.super.ctor(self,data)
    self:init("lua.uiconfig_mango_new.demond.demondOtherArmyVSLayer")
end

function WuLinEnemyVSLayer:initUI(ui)
...
function WuLinEnemyVSLayer:setBossData(userData)
...
function WuLinEnemyVSLayer:onShow()
...
function WuLinEnemyVSLayer:refreshBaseUI()

end

function WuLinEnemyVSLayer:refreshUI()
...
function WuLinEnemyVSLayer:getRoleBtPos(pos)
...
function WuLinEnemyVSLayer.cellClickHandle(sender)
...
function WuLinEnemyVSLayer.onArmyClickHandle(sender)
    local self = sender.logic
...
function WuLinEnemyVSLayer:getChangeBtn()
    return self.btn_challenge
end

--注册事件
function WuLinEnemyVSLayer:registerEvents()
   ...
   --ADD_ALERT_CLOSE_LISTENER(self,self.btn_close)
    ...
    self.btn_close:addMEListener(TFWIDGET_CLICK, audioClickfun(closeFunc),1);
...
function WuLinEnemyVSLayer:removeEvents()
...
function WuLinEnemyVSLayer:drawLeftArea()
    self.button_self = {}
    self.button = {}
    for i=1,9 do
...
function WuLinEnemyVSLayer:drawRightArea()
    self.button = {}
    for i=1,9 do
        local btnName = "panel_item" .. i
...
function WuLinEnemyVSLayer.onBtnAttackClickHandle(sender)
    local self      = sender.logic
    self:Attack()
end

function WuLinEnemyVSLayer:Attack()
    ...
    if activityState ~= WuLinEnemyManager.ActivityState_1 then
        ...
        WuLinEnemyManager:requestBattleStart()

    end 
    AlertManager:closeLayer(self)
end

function WuLinEnemyVSLayer:onClickHandleForTrustLayer()
    self:Attack()
end

function WuLinEnemyVSLayer.cellTouchBeganHandle(cell)
...
function WuLinEnemyVSLayer.cellTouchMovedHandle(cell)
    local self = cell.logic
    self.isMove = true
...
function WuLinEnemyVSLayer.cellTouchEndedHandle(cell)
    local self = cell.logic
...
function WuLinEnemyVSLayer:removeLongTouchTimer()
...
function WuLinEnemyVSLayer:createSelectCussor(cell,pos)
    play_press()
...
function WuLinEnemyVSLayer:moveSelectCussor(cell,pos)
...
function WuLinEnemyVSLayer:releaseSelectCussor(cell,pos)
