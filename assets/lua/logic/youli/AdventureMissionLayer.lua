...
function AdventureMissionLayer:onClickHandleForTrustLayer()

    local newMission = AdventureMissionManager:getCurrentMission(MissionManager.DIFFICULTY3)
    if newMission == nil or (newMission.function_open == 0 or newMission.starLevel ~= MissionManager.STARLEVEL0) then
    ...
    print("newMission = ",newMission.id)
    AdventureManager:openAdventureMissionDetailLayer(newMission.id);
end

function AdventureMissionLayer.onBtnOneKeyClick(sender)
    local self = sender.logic
