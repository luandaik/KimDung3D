...
function AdventureRandomDetailLayer:initUI(ui)
    ...
    local bg_boshu = TFDirector:getChildByPath(ui, "bg_boshu")
    self.totalboshu = TFDirector:getChildByPath(bg_boshu, "txt_2")
    --重置按钮
...
function AdventureRandomDetailLayer:refreshUI()
    ...
    for k=1,1 do
        ...
        local panelCount = 1
        if mission.second_npc ~= "" then
            panelCount = panelCount + 1
            npcSetting = mission.second_npc
        end
        if mission.third_npc ~= "" then
            panelCount = panelCount + 1
            npcSetting = mission.third_npc
        end
        self.totalboshu:setText(panelCount)
...
function AdventureRandomDetailLayer.onMercenaryClickHandle(sender)
    ...
    EmployManager:openRoleList(function ()
        AlertManager:close()
        AlertManager:close()
        MissionManager:attackMission(missionId,EnumFightStrategyType.StrategyType_HIRE_TEAM, true);
    end)
end
