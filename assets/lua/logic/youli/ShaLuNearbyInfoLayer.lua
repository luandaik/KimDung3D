...
function ShaLuNearbyInfoLayer:initUI(ui)
    ...
    --每次切磋的奖励
    local singlePanel = TFDirector:getChildByPath(ui, "panel_res4")
    self.singleQuality = TFDirector:getChildByPath(singlePanel, "bg_icon")
    self.singleIcon = TFDirector:getChildByPath(singlePanel, "img_icon")
    self.singleNum = TFDirector:getChildByPath(singlePanel, "txt_num")
...
function ShaLuNearbyInfoLayer:setInfo(info,clickCallBack)
    ...
    self.txt_lvl:setText(info.level.."d")--stringUtils.format(localizable.common_LV,info.level))
    ...
    if self.reward_item == nil then
        local singleId = ConstantData:objectByID("AdventureKillItemDrop").value or 30038
        local singleNum = ConstantData:objectByID("AdventureKillItemDropNum").value or 5
        local singleInfo = ItemData:objectByID(singleId)

        local rewardInfo = {}
        rewardInfo.type = EnumDropType.GOODS
        rewardInfo.itemId = singleId
        rewardInfo.number = singleNum
        local _rewardInfo = BaseDataManager:getReward(rewardInfo)
        local reward_item =  Public:createIconNumNode(_rewardInfo)
        reward_item:setScale(0.45);
        reward_item:setPosition(self.singleQuality:getPosition())
        reward_item:setZOrder(1)
        self.singleQuality:getParent():addChild(reward_item)
        self.singleQuality:setVisible(false)
