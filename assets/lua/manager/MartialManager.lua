...
MartialManager.MSG_MartialBrushError   	= "MartialManager.MSG_MartialBrushError"	-- 合成部分
...
function MartialManager:ctor(data)
	...
	ErrorCodeManager:addProtocolListener(s2c.MARTIAL_SYNTHESIS_RESULT,  function(target,event)
        if event.errorCode == 0x3402 then
            TFDirector:dispatchGlobalEventWith(MartialManager.MSG_MartialBrushError, {})
            return true
        end
    end)
...
function MartialManager:onMartialLevelUp(event)
	...
	-- TFDirector:dispatchGlobalEventWith("EquipmentChangeEnd",{})
	-- 武学进阶
	TFDirector:dispatchGlobalEventWith(self.MSG_MartialLevelUp, {roleInstance.id})
end
