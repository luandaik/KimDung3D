...
function VipRuleManager:covertToGHType(type)
    ...
    elseif type == EnumRecoverableResType.XILING then
        return HeadResType.XILINGSTONE
    elseif type == EnumRecoverableResType.HERO then
        return HeadResType.WEIWANG
    end
    return 8 + type
end
...
function VipRuleManager:showJiayuanZhuanpanLayer(isadd)
    local resType = EnumRecoverableResType.JIAYUAN_ZHUANPAN
    if self:showItemUseLayer(resType, 30153) then
        return
    end
    toastMessage(localizable.replayLayer_no_jiayuan_zhuanpan)
    -- return self:showBuyLayer(resType,isadd,0,0)
end

--added by wuqi
function VipRuleManager:showXilingLayer(isadd)
    local resType = EnumRecoverableResType.XILING
    
    if self:showItemUseLayer(resType, 30157) then
        return
    end
    
    return self:showBuyLayer(resType, isadd, ConstantData:getValue("Challenge.Time.Climb.NeedVIP"), 0)
end
...
function VipRuleManager:showReplyLayer(type, isadd)
    ...
    elseif type == EnumRecoverableResType.XILING then
        if not self:showXilingLayer(isadd) then
            return
        end 
    elseif type == EnumRecoverableResType.JIAYUAN_ZHUANPAN then
        if not self:showJiayuanZhuanpanLayer(isadd) then
            return
        end 
    elseif type == EnumRecoverableResType.HERO then
        if not self:showHeroMeetingLayer() then
            return
        end 
    end
...
function VipRuleManager:showHeroMeetingLayer()
    local resType = EnumRecoverableResType.HERO
    if self:showItemUseLayer(resType,30612) then
        return
    end
    --toastMessage(localizable.replayLayer_no_jiayuan_zhuanpan)
     return self:showBuyLayer(resType,isadd,0,0)
...
--使用金币消耗
function VipRuleManager:showUsedGoldTimeLayer(todayBuyTime,vipRuleCode, tips)
    local vipInfo = VipData:getVipItemByTypeAndVip(vipRuleCode,MainPlayer:getVipLevel())
    if vipInfo == nil then
        return true
    end
    local maxBuyTime = vipInfo.benefit_value
    if maxBuyTime - todayBuyTime < 1 then
        --判断是否有没有更高的vip能够增加购买次数
        local nextUpVip = VipData:getVipNextAddValueVip(vipRuleCode,MainPlayer:getVipLevel())
        if nextUpVip then
            --local msg = "今日购买次数已用完！\n\n提升至VIP" .. nextUpVip.vip_level .. "可购买" .. nextUpVip.benefit_value .. "次。\n\n是否前往充值？";             
            local msg = stringUtils.format(localizable.VipRuleManager_useGold_vip, tips, nextUpVip.vip_level, nextUpVip.benefit_value)
            CommonManager:showOperateSureLayer(
                    function()
                        PayManager:showPayLayer();
                    end,
            ...
            toastMessage(stringUtils.format(localizable.VipRuleManager_useGold_timeNone, tips))
        end
        return false
    end
    return true
