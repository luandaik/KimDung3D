...
function AlertManager:addLayerToQueue(layer,block,tweentype)
    ...
    if index ~=-1 then 
        ...
        print("Layer already added. It can't be added again. so change to top");
...
function AlertManager:showLayer(layer)
    ...
    if not layer.isShow then
        if not layer.toScene then
            ...
            if (currentScene.__cname == "FightResultScene" or  currentScene.__cname == "FightScene") then
                if (layer.__cname ~= "WarningLayer") then
                    return;
                end
            end
        ...
        if layer.toScene.addLayer then
            ...
            elseif layer.tweentype == AlertManager.TWEEN_2 then
                ...
                for tlayer in self.layerQueue:iterator() do
                    if tlayer.blockUI and not tolua.isnull(tlayer.blockUI) and self:isNeedCloseOtherBlock(layer) then
    ...
    if not tolua.isnull(layer.blockUI) and layer.blockUI.setVisible then
   ...
   -- tempBlock:setBackGroundColor(ccc3(  0,   0,   0));
    ...
    if layer.blockUI then
        if self.layerQueue:length() == 0 then
            ...
            if (topLayer and not tolua.isnull(topLayer)) then
                if topLayer.blockUI and not tolua.isnull(topLayer.blockUI) then
                    topLayer.blockUI:setOpacity(255);
                end
            end
        end
    end
...
-- 清理缓存
function AlertManager:clearAllCacheLayer()
    for v in self.layerCache:iterator() do
