...
function TFScrollRichText:updatePosition( )
	return function(self)
		...
		if self.scrollDirection == self.SCROLLHORIZONTAL then --horizontal
			if self.special and self.special == true then 
				self.Label:setPosition(ccp(self.Label:getPosition().x + (-1)* speed,self.Label:getPosition().y))
				if(self.Label:getPosition().x + self.Label:getSize().width * 0.2  < 0) then
					self.Label:setPosition(ccp(self:getSize().width + self.Label:getSize().width * 0.09,self:getSize().height * 0.5))
...
function TFScrollRichText:setSpecial(bool)
	self.special = bool
end
